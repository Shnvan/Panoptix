...
The production backup workflow, visitor access request workflow, full production sidebar browser smoke, Gateway Discovery V2 backend/edge-agent path, Tailscale RTSP real-camera pilot, and DigitalOcean gateway stability soak have passed or are implemented on `main`. Because production-standard on-site gateway/VLAN rollout is paused until hardware/site access exists, the next no-hardware task is current DigitalOcean pilot monitoring: keep production health checks active, watch admin deep health/gateway heartbeat recency, and treat stale gateway heartbeat, repeated gateway WebSocket reconnect failures, or idle `ffmpeg` on the DigitalOcean host as actionable. Track V380 Pro / V380ProQ16S internet-camera feasibility separately; do not store vendor-cloud credentials, RTSP URLs with credentials, or service tokens in docs, screenshots, frontend code, or browser storage.
