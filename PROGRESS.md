...
- Keep static checks green, document operational changes, and confirm the first scheduled backup/retention run after default-branch activation. Production is live; the main product-path blockers are real LiveKit browser playback, real camera hardware validation, Alerts page API wiring, actor investigation UI, and admin visitor investigation UI.
...
### 1. Commit recurring backup automation
Commit the backup automation implementation after confirming `apps/web/index.html` remains untouched and unstaged. This gives the next session the scheduled workflow, retention job, tests, and updated runbooks.

### 2. Confirm first scheduled backup run
After the workflow lands on `main`, manually dispatch or wait for the daily 18:15 UTC / 02:15 Asia/Manila run. Confirm encrypted backup success, retention success, backup status, and absence of a `Production backup automation failed` issue.
