...
|---|---:|---|---|
| **Frontend** | 94% | Green - Feature-complete, production smoke passed | `integratedCompleteFrontend` is merged into `fullstack-integration`. The React/Vite app has the login shell, viewer dashboard, camera detail modal with real LiveKit subscriber playback (`@livekit/components-react`), admin dashboard, users, cameras, gateways, audit/compliance with full filter controls, alerts wired to real backend APIs, actor investigation UI, visitor investigation UI, DSR, break-glass, health, settings, dev-auth headers, same-origin API client, frontend production proxy server, and same-domain public `/entry` visitor notice flow. Local same-origin smoke and deployed staging browser smoke passed. Production viewer-token smoke passed after PR #20 deploy, PR #21 deployed the subscriber-playback frontend bundle to production (`index-DudfvjqN.js`; old `LiveKit player not wired yet` copy absent from deployed bundle), and full 10-page authenticated production browser smoke completed on 2026-06-01 with no unexpected `404`, `500`, or `502`. Remaining gaps: disabled-account UX verification, Playwright coverage, production polish, and optional Gateway Discovery UI if Ivan reassigns it. |
...
- Visitor access request workflow is implemented in the working tree: `/entry` has a public request form, the backend stores pending `visitor_access_requests` without creating accounts automatically, and admins can review/approve/reject from Users & Access. Approval uses the existing GitHub org invite path and respects disabled-user denial. Before production use, deploy migration `0013_visitor_access_requests` and add only the narrow Cloudflare public exception for `POST /api/v1/visitor/access-requests`.
...
- Keep static checks green, document operational changes, and prepare the real gateway host runbook flow. Production is live; the main product-path blocker is real camera hardware validation. Full 10-page authenticated production browser smoke completed on 2026-06-01 with no unexpected `404`, `500`, or `502`, and the Live Cameras modal showed the new subscriber UI while the tested smoke camera remained degraded at runtime (`Tunnel closed` / `Feed offline`) rather than failing at the API or frontend layer. Frontend UI gaps (Alerts API wiring, actor investigation, visitor investigation, LiveKit subscriber playback, full audit filters) are deployed to production via PR #21; public asset checks confirmed the old token-only modal copy is absent. Nonexistent endpoint calls have been cleaned up.
