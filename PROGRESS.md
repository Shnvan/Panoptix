...
| **DevOps/CI** | 98% | Green - Monitoring hardening in review | GitHub Actions CI covers backend and edge checks. Production health monitoring now requires an explicitly healthy database, LiveKit connection, and gateway heartbeat; degraded or malformed deep health fails closed. A manual-only controlled failure flag verifies sanitized, deduplicated issue creation. Production backup workflow and retention remain active. |
