...
|---|---:|---|---|
| **Frontend** | 98% | Green - Production assistant enabled | PR #27 merged through `713098a` with gateway/camera IDs and pagination, bounded access-request review, alert density controls, visitor filtering, audit ID inspection, and Playwright coverage. Current reliability work exposes hook errors, replaces false empty states with retry panels, and derives camera playback status from LiveKit connection and subscribed video-track events with bounded timeouts. The admin-only operations assistant is enabled in production after provider privacy approval, with disclosure, memory-only history, safe text rendering, desktop/mobile coverage, and no browser-side provider credentials. Gateway Discovery UI remains out of scope. |
| **Backend/API** | 97% | Green - Production assistant enabled | FastAPI control plane is production deployed with authenticated user/admin APIs, gateway APIs, session management, camera/gateway lifecycle, viewer/gateway LiveKit token minting, audit/export/verify, alerts, DSR, visitor collector, actor profile/activity, health probes, backup status, Gateway Discovery V2 admin/gateway endpoints, and a server-side OpenAI-compatible operations assistant. Assistant context is sanitized and read-only, requests are rate limited, provider credentials remain backend-only, and audit rows store metadata only. Gateway control auth, LiveKit webhook handling, viewer tokens, and publish commands supported the real `Tailscale RTSP Camera` pilot. |
