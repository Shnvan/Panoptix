...
| **Infrastructure** | 100% | Green - Temporary API compression mitigation active | **Production live at `panoptix.site`.** Cloudflare temporarily disables compression for `starts_with(http.request.uri.path, "/api/v1/")` after browsers reported malformed compressed JSON responses. This does not broaden Cloudflare Access and is separate from the narrow access-request WAF rule. Root-cause investigation and production-standard physical gateway rollout remain. |
...
- Keep the temporary Cloudflare API compression-disable rule active while investigating the malformed compressed-response source. Verify browser fetches do not report `ERR_CONTENT_DECODING_FAILED`; do not confuse this broad compression rule with a broad public Access exception.
