...
|---|---:|---|---|
| **Frontend** | 94% | Green - Feature-complete, production smoke in progress | `integratedCompleteFrontend` is merged into `fullstack-integration`. The React/Vite app has the login shell, viewer dashboard, camera detail modal with real LiveKit subscriber playback (`@livekit/components-react`), admin dashboard, users, cameras, gateways, audit/compliance with full filter controls, alerts wired to real backend APIs, actor investigation UI, visitor investigation UI, DSR, break-glass, health, settings, dev-auth headers, same-origin API client, frontend production proxy server, same-domain public `/entry` visitor notice flow, and locally QA-covered visitor access request frontend states. Local same-origin smoke, deployed staging browser smoke, frontend guardrail scan, build, lint, and Playwright desktop/mobile access workflow tests pass. Production viewer-token smoke passed after PR #20 deploy, but the currently deployed production modal still shows token-only copy (`LiveKit player not wired yet`). Remaining gaps: full 10-page production browser smoke at `panoptix.site`, production visitor access request smoke after migration `0013` + narrow Cloudflare exception, production polish, and optional Gateway Discovery UI if Ivan reassigns it. |
...
| **Database** | 100% | Green - Strong | Core schema plus migrations through `0012_gateway_discovery_runs` are part of the production path. Production Neon is at Alembic head. Key tables for alerts, notifications, gateway command queue, discovery snapshots, publish state, backup runs, login baselines, visitor visits, and audit are verified. Missing: pilot+ backup retention tables only if richer retention evidence is later required. |
