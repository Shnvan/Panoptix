...
|---|---:|---|---|
| **Frontend** | 98% | Green - Assistant trust upgrade in review | The admin-only assistant remains disclosure-gated and memory-only. The trust upgrade displays backend-generated snapshot time, deterministic evidence, approved runbook references, and a copy control while retaining safe text rendering and no browser-side provider credentials or conversation persistence. Gateway Discovery UI remains out of scope. |
| **Backend/API** | 97% | Green - Assistant trust upgrade in review | The read-only assistant now uses versioned allowlisted runbook guidance instead of changing production facts in its system prompt, isolates prior client transcript entries as untrusted system data, expands secret redaction, and records latency/usage counts without content. API evidence and references are generated deterministically from the sanitized snapshot, not from model claims. |
