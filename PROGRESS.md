...
- Keep static checks green, run an isolated restore drill when R2 backup artifacts are available, and prepare production deployment hardening. Staging deployed browser smoke is now complete. The main product-path blockers are real LiveKit browser playback, real camera hardware validation, Alerts page API wiring, and production hardening.
