...
### 1. Finish visitor access request edge-case smoke
Core production visitor access request smoke passed: migration `0013_visitor_access_requests` is applied, the narrow Cloudflare public `POST /api/v1/visitor/access-requests` exception is active, public request creation and duplicate-pending handling work, and Users & Access reject clears the pending request. Next, smoke approval/GitHub invite with a safe real test identity and verify disabled-user approval returns readable `user-disabled` without re-enabling or inviting.
