...
|---|---:|---|---|
| **Frontend** | 94% | Green - Feature-complete, production smoke passed | `integratedCompleteFrontend` is merged into `fullstack-integration`. The React/Vite app has the login shell, viewer dashboard, camera detail modal with real LiveKit subscriber playback (`@livekit/components-react`), admin dashboard, users, cameras, gateways, audit/compliance with full filter controls, alerts wired to real backend APIs, actor investigation UI, visitor investigation UI, DSR, break-glass, health, settings, dev-auth headers, same-origin API client, frontend production proxy server, and same-domain public `/entry` visitor notice flow. Local same-origin smoke, deployed staging browser smoke, production viewer-token smoke, visitor access-request final smoke, and full 10-sidebar-page authenticated production browser smoke have passed. Production smoke observed only transient recovered `401` bootstrap calls and browser-extension console noise; storage contained no sensitive token material. Remaining gaps: real camera playback validation, Playwright coverage, production polish if later smoke finds issues, and optional Gateway Discovery UI if Ivan reassigns it. |
| **Backend/API** | 96% | Green - Strong | FastAPI control plane is production deployed with authenticated user/admin APIs, gateway APIs, session management, camera/gateway lifecycle, viewer/gateway LiveKit token minting, audit/export/verify, alerts, DSR, visitor collector, actor profile/activity, health probes, backup status, and Gateway Discovery V2 admin/gateway endpoints. Missing: hardware-backed real camera validation and any future API refinements needed by frontend UI gaps. |
| **Edge/Gateway** | 88% | Yellow - Test-verified, hardware pending | Edge agent supports authenticated production heartbeats through Cloudflare Access, stable user agent, gateway control WebSocket/heartbeat command fallback, signed command verification, optional `livekit-ffmpeg` publishing path, supervisor/service templates, credential-file handling, and Gateway Discovery V2 enrichment with approved-CIDR TCP probes plus safe ARP/MAC, mDNS, SSDP/UPnP, and HTTP/HTTPS evidence labels. Missing: real gateway host installation, real camera LAN/VLAN validation, and real CCTV hardware onboarding. |
| **Media/LiveKit** | 88% | Yellow - Cloud ready, browser done, hardware pending | LiveKit Cloud APAC is provisioned and backend token/webhook/room-control paths exist. Synthetic FFmpeg-to-LiveKit smoke passed, command-driven gateway publishing path is scaffolded, CSP supports LiveKit origins, and frontend browser subscriber playback is implemented using `@livekit/components-react` (subscriber-only). Missing: real camera-to-LiveKit publishing from physical hardware and fallback LiveKit operationalization. |
...
| **Infrastructure** | 100% | Green - Strong | **Production live at `panoptix.site` (2026-05-22).** Cloudflare Access "Panoptix Production" protects the app root and protected APIs with GitHub org policy. `/entry` public visitor flow is scoped narrowly. Railway production backend/frontend, Neon production, DNS, production healthcheck, R2 encrypted backups, backup/retention workflow, and LiveKit Cloud are active. Missing: physical gateways and break-glass YubiKey hardware. |
| **Security** | 92% | Green - Strong | CF Access JWT, CSRF, HMAC audit chain, classified audit events with session/IP/UA metadata, audit-of-audit, rate limiting, security headers, gateway service tokens, RBAC, break-glass software path, SCA/SAST CI, mediamtx threat model, mTLS cert bootstrap scaffold, CT-log monitoring, fail-closed secret handling, and production browser storage smoke are done. Missing: device posture enforcement production activation and future hardware smoke evidence. |
...
| **Documentation** | 97% | Green - Strong | Full system plan, API reference, 51+ docs, runbooks, mediamtx threat model, Terraform state security doc, frontend/database guidance, and full-stack status docs are maintained. Gateway Discovery V2 status is reflected in progress and handoff docs. |

### Critical Path to MVP

| # | Blocker | Owner | Depends On |
|---|---------|-------|------------|
| 1 | ~~Real LiveKit browser subscriber playback~~ | Frontend/system owner | ✅ Done — CameraDetailModal uses `@livekit/components-react` subscriber-only viewer; production camera validation pending |
...
Full 10-sidebar-page authenticated production browser smoke is complete: Dashboard, Live Cameras, Camera Management, Gateways, Users & Access, Audit Logs, Alerts, System Health, Break Glass, and Settings loaded through `panoptix.site`; no unexpected `404`/`500`/`502` appeared; transient recovered `401` bootstrap calls were observed; console output was browser-extension/content-script noise only; and storage contained no sensitive token material. Next, install/configure the edge agent on the real gateway host, store raw gateway and Cloudflare service-token secrets only on that host, and confirm production heartbeat through `panoptix.site`.
