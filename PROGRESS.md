...
|---|---:|---|---|
| **Frontend** | 92% | Green - Feature-complete, production smoke in progress | `integratedCompleteFrontend` is merged into `fullstack-integration`. The React/Vite app has the login shell, viewer dashboard, camera detail modal with real LiveKit subscriber playback (`@livekit/components-react`), admin dashboard, users, cameras, gateways, audit/compliance with full filter controls, alerts wired to real backend APIs, actor investigation UI, visitor investigation UI, DSR, break-glass, health, settings, dev-auth headers, same-origin API client, frontend production proxy server, and same-domain public `/entry` visitor notice flow. Local same-origin smoke and deployed staging browser smoke passed. Production viewer-token smoke passed after PR #20 deploy, and PR #21 deployed the subscriber-playback frontend bundle to production (`index-DudfvjqN.js`; old `LiveKit player not wired yet` copy absent from deployed bundle). Remaining gaps: full 10-page authenticated production browser smoke at `panoptix.site`, disabled-account UX verification, Playwright coverage, production polish, and optional Gateway Discovery UI if Ivan reassigns it. |
...
| **Security** | 90% | Green - Strong | CF Access JWT, CSRF, HMAC audit chain, classified audit events with session/IP/UA metadata, audit-of-audit, rate limiting, security headers, gateway service tokens, RBAC, break-glass software path, SCA/SAST CI, mediamtx threat model, mTLS cert bootstrap scaffold, CT-log monitoring, and fail-closed secret handling are done. Missing: device posture enforcement production activation and browser security smoke evidence. |
...
### 1. Finish production browser/security smoke
Visitor access request production smoke is complete for the core and security-critical paths: migration `0013_visitor_access_requests` is applied, the narrow Cloudflare public `POST /api/v1/visitor/access-requests` exception is active, public request creation and duplicate-pending handling work, public admin-role attempts are stored as viewer, approval sends/records a GitHub org invite as viewer, disabled-user approval returns readable `user-disabled` without re-enabling or inviting, and smoke pending requests were cleaned up. Next, run full 10-sidebar-page authenticated production smoke and remaining readable error/rate-limit checks.
