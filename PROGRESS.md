...
|---|---:|---|---|
| **Frontend** | 80% | Yellow - Integrated, UI gaps remain | `integratedCompleteFrontend` is merged into `fullstack-integration`. The React/Vite app has the login shell, viewer dashboard, camera detail modal, admin dashboard, users, cameras, gateways, audit/compliance, DSR, break-glass, health, settings, dev-auth headers, same-origin API client, frontend production proxy server, and same-domain public `/entry` visitor notice flow. Local same-origin smoke and deployed browser smoke passed. Required frontend coworker gaps: real LiveKit subscriber playback using viewer tokens only, Alerts page real API wiring, admin visitor investigation UI, actor investigation UI, full audit filters, Playwright coverage, production polish, and optional Gateway Discovery UI if Ivan reassigns it. |
...
| **Media/LiveKit** | 82% | Yellow - Cloud ready, browser/hardware pending | LiveKit Cloud APAC is provisioned and backend token/webhook/room-control paths exist. Synthetic FFmpeg-to-LiveKit smoke passed, command-driven gateway publishing path is scaffolded, and CSP supports LiveKit origins. Missing: real browser subscriber playback in frontend, real camera-to-LiveKit publishing from physical hardware, and fallback LiveKit operationalization. |
...
| **Privacy/Compliance** | 90% | Green - Strong | Public `/entry` notice flow, signed visitor cookie correlation, visitor signal boundaries, DSR APIs, DPA/export/signage docs, privacy runbooks, audit export/signing, and sanitized discovery constraints are implemented. Missing: frontend investigation UI surfaces and formal pilot evidence packaging. |
