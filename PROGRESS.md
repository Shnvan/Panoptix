...
|---|---:|---|---|
| **Frontend** | 97% | Green - Feature-complete, production and real-camera pilot smoke passed | `integratedCompleteFrontend` is merged into `fullstack-integration`. The React/Vite app has the login shell, viewer dashboard, camera detail modal with real LiveKit subscriber playback (`@livekit/components-react`), admin dashboard, users, cameras, gateways, audit/compliance with full filter controls, alerts wired to real backend APIs, actor investigation UI, visitor investigation UI, DSR, break-glass, health, settings, dev-auth headers, same-origin API client, frontend production proxy server, and same-domain public `/entry` visitor notice flow. Local same-origin smoke, deployed staging browser smoke, production viewer-token smoke, visitor access-request final smoke, full 10-sidebar-page authenticated production browser smoke, the `Tailscale RTSP Camera` production playback pilot, frontend guardrail scan, build, lint, and Playwright desktop/mobile access workflow tests have passed. Production smoke observed only transient recovered `401` bootstrap calls and browser-extension console noise; storage contained no sensitive token material. Remaining gaps: production polish if later smoke finds issues, and optional Gateway Discovery UI if Ivan reassigns it. |
| **Backend/API** | 97% | Green - Strong | FastAPI control plane is production deployed with authenticated user/admin APIs, gateway APIs, session management, camera/gateway lifecycle, viewer/gateway LiveKit token minting, audit/export/verify, alerts, DSR, visitor collector, actor profile/activity, health probes, backup status, and Gateway Discovery V2 admin/gateway endpoints. Gateway control auth, LiveKit webhook handling, viewer tokens, and publish commands supported the real `Tailscale RTSP Camera` pilot. Missing: future API refinements only if later camera/gateway smoke finds issues. |
...
| **Infrastructure** | 100% | Green - Strong | **Production live at `panoptix.site` (2026-05-22).** Cloudflare Access "Panoptix Production" protects the app root and protected APIs with GitHub org policy. `/entry` public visitor flow is scoped narrowly. Railway production backend/frontend, Neon production, DNS, production healthcheck, R2 encrypted backups, backup/retention workflow, LiveKit Cloud, and the temporary DigitalOcean always-on gateway pilot are active. Missing: production-standard dedicated physical gateway rollout and break-glass YubiKey hardware. |
...
| **DevOps/CI** | 98% | Green - Strong | GitHub Actions CI covers backend and edge checks. Production health check targets `panoptix.site` with Cloudflare Access service-token headers and direct GitHub REST issue filing without `actions/github-script`. Production backup workflow and retention are active. Production deploy workflow is manual-ready. Dependabot auto-merge workflow exists for minor/patch updates. |
| **Documentation** | 98% | Green - Strong | Full system plan, API reference, 51+ docs, runbooks, mediamtx threat model, Terraform state security doc, frontend/database guidance, and full-stack status docs are maintained. Gateway Discovery V2, visitor access requests, full sidebar smoke, and Tailscale RTSP real-camera pilot status are reflected in progress and handoff docs. |
...
- Production-standard on-site gateway/VLAN rollout and additional camera sites -> hardware/site access
