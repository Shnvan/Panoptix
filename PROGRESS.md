...
|---|---:|---|---|
| **Frontend** | 98% | Green - Assistant trust upgrade deployed | The admin-only assistant remains disclosure-gated and memory-only. Production includes the `LeBron Yves Saint Laurent D. Uchiha Gojo` name, backend-generated snapshot time, deterministic evidence, approved runbook references, and a copy control. Ivan reported the five administrator prompt checks passed on 2026-06-08. Browser storage and viewer-absence checks remain to be recorded manually. Gateway Discovery UI remains out of scope. |
| **Backend/API** | 97% | Green - Assistant trust upgrade deployed | The read-only assistant uses versioned allowlisted runbook guidance, isolates prior client transcript entries as untrusted data, expands secret redaction, and records latency/usage counts without content. PR #36 merged as `740006d`; production health run `27092024634` passed; the five administrator prompt checks passed. Audit-row metadata evidence remains to be recorded manually. |
| **Edge/Gateway** | 94% | Green - Tailscale RTSP pilot verified | Edge agent supports authenticated production heartbeats through Cloudflare Access, stable user agent, gateway control WebSocket/heartbeat command fallback, signed command verification, optional `livekit-ffmpeg` publishing path, supervisor/service templates, credential-file handling, and Gateway Discovery V2 enrichment with approved-CIDR TCP probes plus safe ARP/MAC, mDNS, SSDP/UPnP, and HTTP/HTTPS evidence labels. The DigitalOcean `dropletGateway` now runs `panoptix-edge-agent.service` as the always-on one-month pilot gateway, pulls the camera over Tailscale/private networking, and publishes to LiveKit only when a viewer opens the camera. 2026-06-02 validation showed the service active, one supervisor process, `ffmpeg` only while the tunnel was active, and no repeated post-restart stale-session failures. Missing: production-standard on-site gateway/VLAN hardening and optional discovery UI if reassigned. |
| **Media/LiveKit** | 94% | Green - Real-camera pilot verified | LiveKit Cloud APAC is provisioned and backend token/webhook/room-control paths exist. Synthetic FFmpeg-to-LiveKit smoke passed, command-driven gateway publishing path is scaffolded, CSP supports LiveKit origins, frontend browser subscriber playback is implemented using `@livekit/components-react` (subscriber-only), and the `Tailscale RTSP Camera` streamed successfully through LiveKit into Panoptix. Browser smoke showed no camera/mic prompt and no browser media publishing. Missing: longer stability evidence and fallback LiveKit operationalization. |
| **Database** | 100% | Green - Strong | Core schema plus migrations through `0013_visitor_access_requests` are part of the production path. Production Neon is at Alembic head. Key tables for alerts, notifications, gateway command queue, discovery snapshots, publish state, backup runs, login baselines, visitor visits, visitor access requests, and audit are verified. Missing: pilot+ backup retention tables only if richer retention evidence is later required. |
| **Infrastructure** | 100% | Green - Compression fix awaiting deployment evidence | **Production live at `panoptix.site`.** The Node frontend proxy fix requests identity encoding upstream and removes stale representation headers, with plain/gzip/Brotli regression coverage. Cloudflare still temporarily disables compression for `/api/v1/*`; keep that rule until the fix is deployed and compressed curl plus authenticated browser checks pass. This does not broaden Cloudflare Access and is separate from the narrow access-request WAF rule. |
| **Security** | 94% | Green - Strong | CF Access JWT, CSRF, HMAC audit chain, classified audit events with session/IP/UA metadata, audit-of-audit, rate limiting, security headers, gateway service tokens, RBAC, break-glass software path, SCA/SAST CI, mediamtx threat model, mTLS cert bootstrap scaffold, CT-log monitoring, fail-closed secret handling, production browser storage smoke, and real-camera pilot browser security smoke are done. The pilot kept RTSP on Tailscale/private networking, and browser storage/logs did not expose RTSP URLs, gateway tokens, Cloudflare tokens, or LiveKit secrets. Missing: device posture enforcement production activation and future site/hardware evidence. |
| **Privacy/Compliance** | 95% | Green - Strong | Public `/entry` notice flow, signed visitor cookie correlation, visitor signal boundaries, DSR APIs, DPA/export/signage docs, privacy runbooks, audit export/signing, sanitized discovery constraints, and frontend admin visitor investigation UI are implemented. Missing: formal pilot evidence packaging. |
| **Backup/DR** | 100% | Green - Strong | Encrypted R2 backup path, dry-run decrypt/`pg_restore --list`, isolated restore-drill evidence, backup status reporting, scheduled production backup workflow, and R2 retention job have succeeded. Continue monitoring scheduled runs and quarterly restore drills. Never restore into production Neon. |
| **DevOps/CI** | 98% | Green - Monitoring hardening active | GitHub Actions CI covers backend, frontend, and edge checks. Production health monitoring requires an explicitly healthy database, LiveKit connection, gateway heartbeat, and enabled assistant; degraded or malformed deep health fails closed. The manual controlled failure test created one sanitized deduplicated issue across two failures, and recovery run `27084083277` passed. |
| **Documentation** | 99% | Green - Current through 2026-06-07 | Handoff, frontend guidance, assistant boundaries, manual testing, monitoring behavior, and Cloudflare runbooks reflect the current release candidate. |

### Critical Path to MVP

| # | Blocker | Owner | Depends On |
|---|---------|-------|------------|
| 1 | ~~Real LiveKit browser subscriber playback~~ | Frontend/system owner | Done - CameraDetailModal uses `@livekit/components-react` subscriber-only viewer; production Tailscale RTSP camera pilot passed |
...
| 3 | ~~Real camera -> LiveKit publishing from edge agent~~ | System owner | Done for Tailscale RTSP pilot - DigitalOcean `dropletGateway` published the Tailscale-private RTSP camera into LiveKit; production-standard on-site gateway/VLAN hardening remains future work |
...
- The admin operations assistant is enabled in production after provider privacy approval. Keep all `AI_ASSISTANT_*` provider credentials backend-only, preserve sanitized read-only context and metadata-only audit behavior, and never place the provider key or endpoint in frontend variables.
- Assistant evidence and runbook references must remain backend-generated from the sanitized snapshot and approved allowlist. Do not accept provider-generated paths or citations as trusted evidence.
- Assistant manual prompt evidence is partially complete: the five production administrator prompts passed on 2026-06-08, including read-only refusal and secret-extraction refusal. Still record browser storage, viewer absence, and audit metadata checks. Operational follow-up remains for `111` open alerts and `4` stale/never-seen gateways surfaced by the assistant snapshot.
- Keep the temporary Cloudflare API compression-disable rule active until the frontend proxy fix is deployed and verified. The fix requests `Accept-Encoding: identity` upstream and strips `Content-Encoding`/`Content-Length` from decoded proxy responses. Verify compressed curl and authenticated browser fetches before disabling the rule; do not confuse this compression rule with a broad public Access exception.
- Frontend list hooks must preserve the distinction between a successful empty response and a failed request. Gateway, camera, user, audit, DSR, and session views should show retryable failure panels when their APIs fail.
- Visitor access request workflow is live for the core production path: `/entry` has a public request form, the backend stores pending `visitor_access_requests` without creating accounts automatically, and admins can review/approve/reject from Users & Access. Production migration `0013_visitor_access_requests` is applied, the narrow Cloudflare public exception for `POST /api/v1/visitor/access-requests` is active, public create returned `201 pending`, duplicate submit returned `409 access-request-already-pending`, admin reject cleared the first pending smoke request, `/entry?mode=request-access` is present in the deployed frontend bundle, and a manual public payload attempting `requested_role: "admin"` was stored as `requested_role: "viewer"` in production. Final access-request smoke on 2026-06-01 confirmed approval sends/records a GitHub org invite as viewer, disabled-user approval returns `409 user-disabled`, the disabled local user remains disabled, no invite metadata is written on the denied request, and smoke pending requests were cleaned up.
...
- Keep static checks green and document operational changes. Production is live; full 10-sidebar-page authenticated production browser smoke passed at `panoptix.site`: all sidebar pages loaded, no unexpected `404`/`500`/`502` appeared, transient `401` bootstrap calls recovered to `200`, console output was browser-extension/content-script noise only, and `localStorage`/`sessionStorage` contained no sensitive token material. The `Tailscale RTSP Camera` production playback pilot passed on 2026-06-02 through the DigitalOcean `dropletGateway`; RTSP stayed on Tailscale/private networking, the browser subscribed to LiveKit only, no camera/mic prompt or browser publishing was observed, and no RTSP URL, gateway token, Cloudflare token, or LiveKit secret was exposed in browser storage/logs/docs. Frontend UI gaps (Alerts API wiring, actor investigation, visitor investigation, LiveKit subscriber playback, full audit filters) are deployed to production via PR #21; public asset checks confirmed the old token-only modal copy is absent. Nonexistent endpoint calls have been cleaned up.
...
- Production-standard on-site gateway/VLAN rollout and additional camera sites -> hardware/site access. Operator-laptop checks on 2026-06-06 could not reach `100.76.147.59` or TCP `8554`; continue from the gateway host and camera LAN/VLAN.
...
- **Frontend implementation** (`apps/web/`) is explicitly reassigned for the current reliability and UI cleanup work. Gateway Discovery UI remains out of scope unless separately reassigned.
...
- [x] `participant_left` with zero viewers now schedules a delayed stop when the maintenance scheduler is enabled, and immediately enqueues `stop_publish` when the scheduler is disabled
...
### 1. Current DigitalOcean pilot monitoring
Full 10-sidebar-page authenticated production browser smoke is complete, and the `Tailscale RTSP Camera` production playback pilot passed on 2026-06-02 through the DigitalOcean `dropletGateway`. The production idle -> playback -> idle soak also passed on 2026-06-02 after the backend mitigation deployed on production commit `cee14ad`. Store raw gateway and Cloudflare service-token secrets only on the gateway host.

2026-06-02 soak execution status: passed. SSH with the local `panoptix_do` key reached the Tailscale-routed DigitalOcean gateway as `root`; no env files, tokens, RTSP URLs, or service secrets were read. Idle evidence showed `panoptix-edge-agent.service` active, `NRestarts=0`, `ExecMainPID=46735`, start time `2026-06-02 06:48:43 UTC`, exactly one edge-agent supervisor process, and no idle `ffmpeg`. During production playback, one `ffmpeg` child (`PID 50457`, parent `46735`) appeared under the supervisor and the RTSP URL remained redacted in written evidence. After closing playback and waiting briefly, only supervisor `46735` remained and `ffmpeg` had exited. Browser smoke stayed subscriber-only with no camera/mic permission prompt; `localStorage` contained only `panoptix-theme=dark`, `sessionStorage` was empty, and no RTSP URL, gateway token, Cloudflare secret, LiveKit secret, or auth token was exposed in browser storage.
...
2026-06-07 release evidence: PR #31 merged as `4155f91`; the deployed `/entry` bundle contains the operations assistant code. Controlled health runs `27084039155` and `27084060783` produced one deduplicated sanitized issue (#32), recovery run `27084083277` passed, and issue #32 was closed with the run links. The monitor initially required the assistant disabled; after provider privacy approval and production enablement later on June 7, the required production state changed to enabled. A read-only DigitalOcean check found `panoptix-edge-agent.service` active, `NRestarts=0`, `ExecMainPID=70456`, one supervisor, zero idle `ffmpeg`, and zero matching stale-session/auth/LiveKit/WebSocket/reconnect failure lines in the preceding 24 hours.
...
### 3. V380ProQ16S internet camera feasibility
...
### 4. Production security evidence
Continue production evidence work for WARP/device posture activation and future targeted hardware/browser evidence only when new deploys or real-device smoke require it. Keep Cloudflare Access, `/entry`, protected APIs, disabled-user enforcement, and visitor collector boundaries unchanged unless a specific security task requires a documented change.

### 5. External blockers
Track production-standard on-site gateway/VLAN rollout, additional camera sites, and break-glass YubiKey hardware as external blockers. The Tailscale RTSP pilot camera path is already validated; new camera/site onboarding should repeat the same no-secret browser and gateway evidence checks.

### 6. Frontend coworker handoff
Keep Alerts real API UI, admin visitor investigation UI, actor investigation UI, real LiveKit subscriber playback, and full audit filters assigned to the frontend coworker unless Ivan explicitly reassigns frontend work. The real-camera playback pilot passed; frontend camera work should continue only if later smoke finds UI issues. Gateway Discovery UI is optional future frontend work only; backend/edge discovery exists, but no frontend discovery UI is required yet.
...
- [x] Historical staging health verification passed: `https://staging.panoptix.site/health` returned `{"status":"ok"}` behind Cloudflare Access
- [x] GitHub OAuth login flow working end-to-end
- [x] 7-day uptime gate completed after starting 2026-05-13
- [x] Production healthcheck workflow added for `https://panoptix.site/health` and `https://panoptix.site/api/v1/admin/health/deep`
- [x] Old staging healthcheck workflow removed so scheduled monitoring no longer targets `staging.panoptix.site`
- [x] Healthcheck workflow sends Cloudflare Access service-token headers, avoids raw response bodies in `$GITHUB_OUTPUT`, and files deduped GitHub issues through the REST API without `actions/github-script`
...
### Frontend UI Gaps Closed - 2026-05-30

- [x] **Alerts page** (`AlertsPanel.tsx`) wired to real backend alert APIs: `GET /api/v1/admin/alerts` with status/severity/category filters, detail view, `POST acknowledge`, and `POST resolve` actions
- [x] **Actor investigation** (`ActorInvestigationPage.tsx`) implemented: profile panel with identity/roles/sessions/camera access/stream grants/alert summary/login baseline, activity timeline with cursor pagination and all backend-supported filters
- [x] **Visitor investigation** (`VisitorInvestigationPage.tsx`) implemented: visit list with pagination, detail drawer showing all 8 documented sections (visitor summary, IP/location, device/browser, network hints, WebRTC check, timing, server context, risk context), risk flags banner, login correlation
- [x] **LiveKit subscriber playback** (`CameraDetailModal.tsx`) implemented: uses `@livekit/components-react` and `livekit-client` as subscriber-only viewer with `audio={false} video={false}` — no browser media publishing
- [x] **Full audit filter controls** (`AuditLogTable.tsx`) implemented: all 10 backend-supported filter parameters (action, actor_type, actor_id, severity, category, outcome, resource, session_id, ts_from, ts_to) plus client-side text search
- [x] **Nonexistent endpoint calls cleaned up**: `listSites` commented out with explicit note; `runExposureCheck`, `runMediaIsolationCheck`, `runOriginBindingCheck` removed from API client
- [x] **Security guardrails verified**: no `getUserMedia`, `MediaRecorder`, `publishTrack`, or auth in `localStorage`/`sessionStorage`; CSRF `x-panoptix-csrf-token` header correctly sent on all mutations; `localStorage` only stores theme preference with explicit guardrail comment
- [x] Updated all frontend documentation files to reflect current implementation state

### Visitor Access Frontend QA - 2026-06-04

- [x] Polished `/entry` request-access form with accessible labels, invalid-field hints, live status messaging, and explicit duplicate pending, rate-limit, validation, backend unavailable, and network failure states
- [x] Replaced Users & Access browser prompt/confirm access-request decisions with in-app approve/reject dialogs, action progress, success/error messaging, disabled-user handling, and load-more support for `next_cursor`
- [x] Added Playwright + axe coverage for public `/entry` access requests and admin access-request review flows across desktop and mobile Chromium
- [x] Added frontend guardrail scan for forbidden browser media APIs, token storage, gateway-only routes, Gateway Discovery UI calls, and obvious frontend secret leaks
- [x] Verified `npm run lint`, `npm run build`, `npm run qa:guardrails`, and `npx playwright test`; production smoke still waits on migration `0013_visitor_access_requests` and the narrow public Cloudflare exception for `POST /api/v1/visitor/access-requests`

### Production Backup Evidence - 2026-05-25
...
- Recurring backup automation now exists as `.github/workflows/production-backup.yml`; scheduled runs begin only after the workflow lands on the default branch.
- R2 object retention now exists as `python -m cctv_api.jobs.backup_retention_r2`; it keeps all backups newer than 30 days, one monthly backup for 12 months, preserves unparseable `.dump.age` keys, and deletes only encrypted R2 objects.
- Next backup action: confirm the first scheduled backup/retention run and keep quarterly isolated restore drills current; do not store private `age` identities in Railway, GitHub, or the repo.
