...
|---|---:|---|---|
| **Frontend** | 98% | Green - PR #27 cleanup merged; reliability hardening in progress | PR #27 merged through `713098a` with gateway/camera IDs and pagination, bounded access-request review, alert density controls, visitor filtering, audit ID inspection, and Playwright coverage. Current reliability work exposes hook errors, replaces false empty states with retry panels, and derives camera playback status from LiveKit connection and subscribed video-track events with bounded timeouts. An admin-only, disabled-by-default operations assistant widget is implemented with disclosure, memory-only history, safe text rendering, desktop/mobile coverage, and no browser-side provider credentials. Gateway Discovery UI remains out of scope. |
| **Backend/API** | 97% | Green - Strong | FastAPI control plane is production deployed with authenticated user/admin APIs, gateway APIs, session management, camera/gateway lifecycle, viewer/gateway LiveKit token minting, audit/export/verify, alerts, DSR, visitor collector, actor profile/activity, health probes, backup status, Gateway Discovery V2 admin/gateway endpoints, and a disabled-by-default server-side OpenAI-compatible operations assistant. Assistant context is sanitized and read-only, requests are rate limited, and audit rows store metadata only. Gateway control auth, LiveKit webhook handling, viewer tokens, and publish commands supported the real `Tailscale RTSP Camera` pilot. |
...
| **DevOps/CI** | 98% | Green - Monitoring hardening active | GitHub Actions CI covers backend and edge checks. Production health monitoring requires an explicitly healthy database, LiveKit connection, gateway heartbeat, and disabled assistant; degraded or malformed deep health fails closed. The manual controlled failure test created one sanitized deduplicated issue across two failures, and recovery run `27084083277` passed. Latest scheduled health run `27079702256` and backup run `27072130783` succeeded. |
...
- The admin operations assistant is implemented but remains disabled by default. Enable it only after approving the model processor and setting backend-only `AI_ASSISTANT_*` values. Never place the provider key or endpoint in frontend variables.
