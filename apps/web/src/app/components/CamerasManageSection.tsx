          ...
          <form onSubmit={handleCreate} className="space-y-4">
            ...
            <div className="flex gap-2">
              <button type="button" onClick={() => setShowCreateForm(false)}
                className={`px-4 py-2 rounded-lg text-sm ${d ? 'bg-slate-800 text-slate-300' : 'bg-slate-100 text-slate-600'}`}>Cancel</button>
            ...
            return (
              <motion.div key={cam.camera_id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }}
                className={`backdrop-blur-xl border rounded-lg p-5 transition-all hover:shadow-lg ${
                  d ? 'bg-gradient-to-br from-slate-900/90 to-slate-800/90 border-slate-700/50 hover:shadow-orange-500/10' : 'bg-white border-slate-200 hover:shadow-md'
                }`}>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-6 bg-black/50" onClick={() => setAclModal(null)}>
          <div className={`border rounded-lg p-6 max-w-md w-full ${d ? 'bg-slate-900 border-slate-700' : 'bg-white border-slate-200'}`} onClick={(e) => e.stopPropagation()}>
            ...
            <div className="space-y-4">
                ...
                {(['grant', 'revoke'] as const).map((a) => (
                  <button key={a} onClick={() => setAclAction(a)}
                    className={`flex-1 py-2 rounded-lg text-sm font-medium transition-colors ${
                      aclAction === a ? 'bg-orange-500 text-white' : d ? 'bg-slate-800 text-slate-300' : 'bg-slate-100 text-slate-600'
              ...
              <div className="flex gap-2">
                <button onClick={() => setAclModal(null)} className={`flex-1 py-2 rounded-lg text-sm ${d ? 'bg-slate-800 text-slate-300' : 'bg-slate-100 text-slate-600'}`}>Cancel</button>
