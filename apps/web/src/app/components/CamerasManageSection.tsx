import { Camera, Plus, Shield, UserPlus, XCircle, CheckCircle, Search, Trash2 } from 'lucide-react';
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between flex-wrap gap-4">
        <div>
          <h2 className={`text-2xl font-bold mb-1 ${d ? 'text-white' : 'text-slate-900'}`}>Camera Management</h2>
          <p className={d ? 'text-slate-400' : 'text-slate-500'}>Register cameras, manage user access, and retire cameras</p>
        </div>
        <button onClick={() => setShowCreateForm(!showCreateForm)}
          className="flex items-center gap-2 px-4 py-2.5 bg-gradient-to-r from-cyan-500 to-blue-600 hover:from-cyan-400 hover:to-blue-500 text-white rounded-xl shadow-lg shadow-cyan-500/25 transition-all text-sm font-medium">
      ...
      {showCreateForm && (
        <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }}
          ...
          <h3 className={`font-semibold mb-4 flex items-center gap-2 ${d ? 'text-white' : 'text-slate-900'}`}>
            <Camera className="w-5 h-5 text-cyan-500" /> Register New Camera
          </h3>
          <form onSubmit={handleCreate} className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div className="space-y-1.5">
                <label className={`text-sm font-medium ${d ? 'text-slate-300' : 'text-slate-700'}`}>Display Name *</label>
                <input type="text" required value={newName} onChange={(e) => setNewName(e.target.value)}
                  placeholder="e.g. Front Gate" className={`w-full rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-cyan-500/50 ${
                    d ? 'bg-slate-800/50 border border-slate-700/50 text-white placeholder-slate-500' : 'bg-slate-50 border border-slate-200 text-slate-900 placeholder-slate-400'
                  }`} />
              </div>
              <div className="space-y-1.5">
                <label className={`text-sm font-medium ${d ? 'text-slate-300' : 'text-slate-700'}`}>Source Type *</label>
                <select value={newSourceType} onChange={(e) => setNewSourceType(e.target.value as CameraSourceType)}
                  className={`w-full rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-cyan-500/50 ${
                    d ? 'bg-slate-800/50 border border-slate-700/50 text-white' : 'bg-slate-50 border border-slate-200 text-slate-900'
                  }`}>
                  {sourceTypes.map((t) => <option key={t} value={t}>{sourceTypeLabels[t]}</option>)}
                </select>
              </div>
              <div className="space-y-1.5">
                <label className={`text-sm font-medium ${d ? 'text-slate-300' : 'text-slate-700'}`}>LiveKit Room Name *</label>
                <input type="text" required value={newRoomName} onChange={(e) => setNewRoomName(e.target.value)}
                  placeholder="e.g. camera_front_gate" className={`w-full rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-cyan-500/50 ${
                    d ? 'bg-slate-800/50 border border-slate-700/50 text-white placeholder-slate-500' : 'bg-slate-50 border border-slate-200 text-slate-900 placeholder-slate-400'
                  }`} />
              </div>
            </div>
            ...
            <div className="flex gap-2">
              <button type="button" onClick={() => setShowCreateForm(false)}
                className={`px-4 py-2 rounded-xl text-sm ${d ? 'bg-slate-800 text-slate-300' : 'bg-slate-100 text-slate-600'}`}>Cancel</button>
              <button type="submit" disabled={creating}
                className="px-6 py-2 bg-gradient-to-r from-cyan-500 to-blue-600 hover:from-cyan-400 hover:to-blue-500 text-white rounded-xl text-sm font-medium disabled:opacity-50">
      ...
      {/* Camera List */}
      {loading ? (
        <div className="text-center py-12 text-slate-400">Loading cameras...</div>
      ) : cameras.length === 0 ? (
        <div className={`text-center py-16 border rounded-xl ${d ? 'bg-slate-900/50 border-slate-700/50' : 'bg-slate-50 border-slate-200'}`}>
          <Camera className={`w-16 h-16 mx-auto mb-4 ${d ? 'text-slate-600' : 'text-slate-300'}`} />
          <h3 className={`text-lg font-semibold mb-2 ${d ? 'text-white' : 'text-slate-900'}`}>No Cameras Registered</h3>
          <p className={d ? 'text-slate-400' : 'text-slate-500'}>Use the "Register Camera" button above to add your first camera.</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
          {cameras.map((cam, i) => (
            <motion.div key={cam.camera_id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }}
              className={`backdrop-blur-xl border rounded-xl p-5 transition-all hover:shadow-lg ${
                d ? 'bg-gradient-to-br from-slate-900/90 to-slate-800/90 border-slate-700/50 hover:shadow-cyan-500/10' : 'bg-white border-slate-200 hover:shadow-md'
              }`}>
              <div className="flex items-start justify-between mb-3">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h3 className={`font-semibold ${d ? 'text-white' : 'text-slate-900'}`}>{cam.display_name}</h3>
                    <p className={`text-xs ${d ? 'text-slate-400' : 'text-slate-500'}`}>Room: {cam.livekit_room_name}</p>
                  </div>
                </div>
                <span className={`px-2 py-0.5 rounded text-xs font-medium ${
                  d ? 'bg-cyan-500/20 text-cyan-400' : 'bg-cyan-50 text-cyan-700'
                }`}>{cam.source_type || 'rtsp'}</span>
              </div>

              <div className={`text-sm mb-4 ${d ? 'text-slate-400' : 'text-slate-500'}`}>
              ...
              <div className={`flex gap-2 pt-3 border-t ${d ? 'border-slate-700/50' : 'border-slate-100'}`}>
                <button onClick={() => setAclModal({ cameraId: cam.camera_id, displayName: cam.display_name })}
                  className={`flex-1 flex items-center justify-center gap-1.5 px-3 py-2 rounded-xl text-sm transition-colors ${
                    d ? 'bg-slate-800/50 hover:bg-slate-700/50 text-white' : 'bg-slate-100 hover:bg-slate-200 text-slate-700'
                  }`}>
                  <UserPlus className="w-3.5 h-3.5" /> ACL
                </button>
                <button onClick={() => setDisableModal({ cameraId: cam.camera_id, displayName: cam.display_name })}
                  className="flex-1 flex items-center justify-center gap-1.5 px-3 py-2 bg-red-500/10 hover:bg-red-500/20 border border-red-500/20 rounded-xl text-sm text-red-400 transition-colors">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-6 bg-black/50" onClick={() => setAclModal(null)}>
          <div className={`border rounded-2xl p-6 max-w-md w-full ${d ? 'bg-slate-900 border-slate-700' : 'bg-white border-slate-200'}`} onClick={(e) => e.stopPropagation()}>
            <h3 className={`text-lg font-bold mb-2 ${d ? 'text-white' : 'text-slate-900'}`}>
              <Shield className="w-5 h-5 text-cyan-500 inline mr-2" />Camera ACL: {aclModal.displayName}
            </h3>
            <p className={`text-sm mb-4 ${d ? 'text-slate-400' : 'text-slate-500'}`}>Grant or revoke a user's access to this camera.</p>
            <div className="space-y-4">
              <div className="flex gap-2">
                {(['grant', 'revoke'] as const).map((a) => (
                  <button key={a} onClick={() => setAclAction(a)}
                    className={`flex-1 py-2 rounded-xl text-sm font-medium transition-colors ${
                      aclAction === a ? 'bg-cyan-500 text-white' : d ? 'bg-slate-800 text-slate-300' : 'bg-slate-100 text-slate-600'
                    }`}>{a === 'grant' ? 'Grant Access' : 'Revoke Access'}</button>
                ))}
              </div>
              <div className="space-y-1.5">
                <label className={`text-sm font-medium ${d ? 'text-slate-300' : 'text-slate-700'}`}>User Email</label>
                <div className="relative">
                  <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400" />
                  <input type="email" required value={aclEmail} onChange={(e) => setAclEmail(e.target.value)}
                    placeholder="user@school.edu" className={`w-full rounded-xl pl-10 pr-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-cyan-500/50 ${
              ...
              <div className="flex gap-2">
                <button onClick={() => setAclModal(null)} className={`flex-1 py-2 rounded-xl text-sm ${d ? 'bg-slate-800 text-slate-300' : 'bg-slate-100 text-slate-600'}`}>Cancel</button>
                <button onClick={handleAcl} disabled={!aclEmail}
                  className="flex-1 py-2 bg-cyan-500 hover:bg-cyan-400 text-white rounded-xl text-sm font-medium disabled:opacity-50">
            ...
            <div className="space-y-4">
              ...
              <div className="flex gap-2">
                ...
                <button onClick={handleDisable} disabled={!disableReason}
                  className="flex-1 py-2 bg-red-500 hover:bg-red-400 text-white rounded-xl text-sm font-medium disabled:opacity-50">Retire Camera</button>
              </div>
            </div>
          </div>
        </div>
