...
export function GatewayCard({ name, status, gatewayId, commandCount = 0, lastSeen }: GatewayCardProps) {
  ...
  const cfg = {
    ...
    retired: { bg: 'from-slate-700/20 to-slate-800/20', border: 'border-slate-600/30', text: 'text-slate-400', dot: 'bg-slate-400', lightBg: 'from-slate-50 to-slate-100/50', lightBorder: 'border-slate-200', lightText: 'text-slate-600' },
  }[status];
    ...
    >
      <div className="flex items-start justify-between mb-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className={`font-semibold ${theme === 'dark' ? 'text-white' : 'text-slate-900'}`}>{name}</h3>
      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        <div className="space-y-1">
          ...
          <p className={`font-semibold ${theme === 'dark' ? 'text-white' : 'text-slate-900'}`}>{commandCount}</p>
        </div>
        <div className="space-y-1">
          <div className={`flex items-center gap-2 text-sm ${theme === 'dark' ? 'text-slate-400' : 'text-slate-500'}`}>
          ...
          <div className="flex items-center gap-2">
            <div className={`flex-1 h-2 rounded-full overflow-hidden ${theme === 'dark' ? 'bg-slate-700/50' : 'bg-slate-200'}`}>
              <div className={`h-full ${status === 'enabled' ? 'bg-emerald-500' : 'bg-slate-400'}`} style={{ width: status === 'enabled' ? '45%' : '0%' }} />
            </div>
          </div>
        </div>
        <div className="space-y-1">
          ...
          <div className="flex items-center gap-2">
            <div className={`flex-1 h-2 rounded-full overflow-hidden ${theme === 'dark' ? 'bg-slate-700/50' : 'bg-slate-200'}`}>
              <div className={`h-full ${status === 'enabled' ? 'bg-emerald-500' : 'bg-slate-400'}`} style={{ width: status === 'enabled' ? '62%' : '0%' }} />
            </div>
          </div>
        </div>
      </div>
      ...
      <div className={`mt-4 pt-4 border-t flex items-center gap-2 text-sm ${
        theme === 'dark' ? 'border-slate-700/50 text-slate-400' : 'border-slate-200 text-slate-500'
      }`}>
