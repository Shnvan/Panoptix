        ...
        <div className="p-4 bg-amber-500/10 border-2 border-amber-500/30 rounded-lg">
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <p className={`text-sm mb-3 ${d ? 'text-neutral-300' : 'text-neutral-600'}`}>This token is shown <strong>only once</strong>. It will not be displayed again. Store it securely on the gateway device.</p>
              <code className={`block p-3 rounded-md text-sm font-mono break-all ${d ? 'bg-neutral-800 text-emerald-400' : 'bg-neutral-100 text-emerald-700'}`}>{oneTimeToken}</code>
              ...
              <button onClick={() => setOneTimeToken(null)} className="mt-3 ml-2 px-4 py-2 bg-neutral-800 hover:bg-neutral-700 rounded-lg text-neutral-300 text-sm">Dismiss</button>
            </div>
          </div>
        </div>
      )}
          ...
          <form onSubmit={handleCreate} className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="space-y-1.5">
                <label className={`text-sm font-medium ${d ? 'text-neutral-300' : 'text-neutral-700'}`}>Gateway Name *</label>
              ...
              <div className="space-y-1.5">
                <label className={`text-sm font-medium ${d ? 'text-neutral-300' : 'text-neutral-700'}`}>mTLS Fingerprint (optional)</label>
      ...
      {disableModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-6 bg-black/50" onClick={() => setDisableModal(null)}>
          <div className={`border rounded-lg p-6 max-w-md w-full ${d ? 'bg-neutral-900 border-neutral-700' : 'bg-white border-neutral-200'}`} onClick={e => e.stopPropagation()}>
            ...
            <p className={`text-sm mb-4 ${d ? 'text-neutral-300' : 'text-neutral-600'}`}>⚠ This will immediately stop all active publish sessions (within 10 seconds). This action is audited.</p>
