...
export function SettingsSection({ user }: SettingsSectionProps) {
  ...
  return (
    <div className="space-y-6">
      ...
      {msg && (
        <div className={`p-3 rounded-xl text-sm ${msg.includes('Failed') || msg.includes('denied') ? 'bg-red-500/10 border border-red-500/20 text-red-400' : 'bg-emerald-500/10 border border-emerald-500/20 text-emerald-400'}`}>{msg}</div>
      )}
      ...
      <div className={`backdrop-blur-xl border rounded-xl p-6 ${d ? 'bg-gradient-to-br from-slate-900/90 to-slate-800/90 border-slate-700/50' : 'bg-white border-slate-200'}`}>
        <div className="flex items-center gap-3 mb-6">
          <Shield className={`w-5 h-5 ${d ? 'text-cyan-400' : 'text-cyan-600'}`} />
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {[
            ...
            <div key={label} className={`p-3 rounded-xl ${d ? 'bg-slate-800/50' : 'bg-slate-50'}`}>
      ...
      <div className={`backdrop-blur-xl border rounded-xl p-6 ${d ? 'bg-gradient-to-br from-slate-900/90 to-slate-800/90 border-slate-700/50' : 'bg-white border-slate-200'}`}>
        ...
        <div className={`space-y-3 text-sm ${d ? 'text-slate-300' : 'text-slate-600'}`}>
          ...
          <div className="flex items-center gap-2"><div className="w-1.5 h-1.5 rounded-full bg-cyan-500" /> MFA reset is admin-mediated only — you cannot reset your own MFA</div>
        </div>
      </div>
      ...
      <div className={`backdrop-blur-xl border rounded-xl p-6 ${d ? 'bg-gradient-to-br from-slate-900/90 to-slate-800/90 border-slate-700/50' : 'bg-white border-slate-200'}`}>
        <div className="flex items-center gap-3 mb-6">
          <Monitor className={`w-5 h-5 ${d ? 'text-cyan-400' : 'text-cyan-600'}`} />
        ...
        <div className="space-y-3">
          {sessions.length === 0 ? (
            ...
            <div key={session.id} className={`flex items-center justify-between p-4 rounded-xl ${d ? 'bg-slate-800/50' : 'bg-slate-50'}`}>
              <div>
              ...
              <button onClick={() => handleRevokeSession(session.id)} disabled={revokedId === session.id}
                className="flex items-center gap-2 px-3 py-1.5 bg-red-500/10 hover:bg-red-500/20 border border-red-500/20 rounded-xl text-red-400 text-xs font-medium transition-colors disabled:opacity-50">
      ...
      <div className={`backdrop-blur-xl border rounded-xl p-6 ${d ? 'bg-gradient-to-br from-slate-900/90 to-slate-800/90 border-slate-700/50' : 'bg-white border-slate-200'}`}>
        ...
        <div className="space-y-2">
          {sessions.length === 0 ? (
            ...
            <div key={`hist-${session.id}`} className={`flex items-center gap-4 p-3 rounded-xl ${d ? 'bg-slate-800/50' : 'bg-slate-50'}`}>
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2">
                  ...
                  <span className={`text-xs px-2 py-0.5 rounded-lg ${d ? 'bg-emerald-500/20 text-emerald-400' : 'bg-emerald-50 text-emerald-700'}`}>
                    Authenticated
                  </span>
                </div>
