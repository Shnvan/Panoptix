...
export function SettingsSection({ user }: SettingsSectionProps) {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className={`text-2xl font-bold mb-1 ${d ? 'text-white' : 'text-neutral-900'}`}>Settings</h2>
        <p className={d ? 'text-neutral-400' : 'text-neutral-500'}>Manage your profile and active sessions</p>
      </div>
      ...
      <div className={`backdrop-blur-xl border rounded-lg p-6 ${d ? 'bg-gradient-to-br from-neutral-900/90 to-neutral-800/90 border-neutral-700/50' : 'bg-white border-neutral-200'}`}>
        <div className="flex items-center gap-3 mb-6">
          ...
          <h3 className={`font-semibold ${d ? 'text-white' : 'text-neutral-900'}`}>Your Profile</h3>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {[
            ...
            <div key={label} className={`p-3 rounded-lg ${d ? 'bg-neutral-800/50' : 'bg-neutral-50'}`}>
              <p className={`text-xs mb-1 ${d ? 'text-neutral-400' : 'text-neutral-500'}`}>{label}</p>
              <p className={`text-sm font-medium ${d ? 'text-white' : 'text-neutral-900'}`}>{value}</p>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className={`backdrop-blur-xl border rounded-lg p-6 ${d ? 'bg-gradient-to-br from-neutral-900/90 to-neutral-800/90 border-neutral-700/50' : 'bg-white border-neutral-200'}`}>
        <div className="flex items-center gap-3 mb-4">
          ...
          <h3 className={`font-semibold ${d ? 'text-white' : 'text-neutral-900'}`}>Security Information</h3>
        </div>
        <div className={`space-y-3 text-sm ${d ? 'text-neutral-300' : 'text-neutral-600'}`}>
        ...
        <div className="space-y-3">
          {sessionsLoading && sessions.length === 0 ? (
            ...
            <div key={session.id} className={`flex items-center justify-between p-4 rounded-lg ${d ? 'bg-neutral-800/50' : 'bg-neutral-50'}`}>
              <div>
                <p className={`text-sm font-medium font-mono ${d ? 'text-white' : 'text-neutral-900'}`}>{session.id.slice(0, 12)}...</p>
                ...
                {session.ua_fp && <p className={`text-xs mt-1 ${d ? 'text-neutral-500' : 'text-neutral-400'}`}>{session.ua_fp}</p>}
              </div>
      ...
      <div className={`backdrop-blur-xl border rounded-lg p-6 ${d ? 'bg-gradient-to-br from-neutral-900/90 to-neutral-800/90 border-neutral-700/50' : 'bg-white border-neutral-200'}`}>
        <div className="flex items-center gap-3 mb-6">
          ...
          <div>
            <h3 className={`font-semibold ${d ? 'text-white' : 'text-neutral-900'}`}>Login History</h3>
            <p className={`text-sm ${d ? 'text-neutral-400' : 'text-neutral-500'}`}>Recent login sessions for your account</p>
          </div>
        </div>
          ...
          {sessionsLoading && sessions.length === 0 ? (
            ...
            <div key={`hist-${session.id}`} className={`flex items-center gap-4 p-3 rounded-lg ${d ? 'bg-neutral-800/50' : 'bg-neutral-50'}`}>
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className={`text-xs truncate ${d ? 'text-neutral-400' : 'text-neutral-500'}`}>
