...
interface StatCardProps {
  ...
  icon: LucideIcon;
  color: 'blue' | 'emerald' | 'red' | 'amber' | 'cyan';
}
...
const colorMap = {
  ...
  cyan:    { bg: 'from-cyan-500/20 to-cyan-600/20', border: 'border-cyan-500/30', icon: 'text-cyan-400', text: 'text-cyan-400', shadow: 'shadow-cyan-500/20', lightBg: 'from-cyan-50 to-cyan-100/50', lightBorder: 'border-cyan-200', lightIcon: 'text-cyan-600', lightText: 'text-cyan-700' },
};
...
export function StatCard({ title, value, change, trend = 'neutral', icon: Icon, color }: StatCardProps) {
  ...
  return (
    <motion.div
      ...
      className={`backdrop-blur-xl border rounded-xl p-6 transition-all duration-300 cursor-default ${
        theme === 'dark'
