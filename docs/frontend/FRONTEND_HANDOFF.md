...
- Its user-facing name is `LeBron Yves Saint Laurent D. Uchiha Gojo`; keep the visible heading and accessibility labels consistent.
- The assistant trust upgrade adds backend-generated `generated_at`, evidence, and approved runbook references to each response. Display these as untrusted informational guidance, keep them memory-only, and never turn reference paths into arbitrary provider-controlled links. The copy control copies only sanitized response text.
