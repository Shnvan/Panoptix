...
**Current active task (2026-06-01):** run full authenticated production sidebar smoke and polish only verified frontend issues. Visitor access request production smoke is complete for the security-critical paths: migration `0013_visitor_access_requests` is applied, the narrow public `POST /api/v1/visitor/access-requests` Cloudflare exception is active, public create returned `201 pending`, duplicate submit returned `409 access-request-already-pending`, Users & Access reject cleared smoke requests, the deployed frontend includes `/entry?mode=request-access`, a manual public `requested_role: "admin"` payload was stored as `viewer`, approval sent/recorded a GitHub invite as viewer, and disabled-user approval returned readable `user-disabled` without re-enabling or inviting. Remaining access-request work is only rate-limit/readable error polish if found during smoke. Do not start Gateway Discovery UI or V380/V380ProQ16S camera integration from the frontend side unless Ivan explicitly reassigns it.
...
| Implemented; production smoke passed | `/api/v1/admin/access-requests`, detail, approve, reject | Users & Access has an Access Requests review panel | Production smoke confirmed reject cleanup, approve/GitHub invite as viewer, and `409 user-disabled` when the email belongs to a disabled local user. Continue only with visible UI polish found during full sidebar smoke. |
...
| Full production smoke test | In progress | Production viewer-token smoke passed 2026-05-31 after PR #20 deployed, and PR #21 deployed the subscriber-playback frontend. Full 10-sidebar-page authenticated production smoke through Cloudflare Access remains required. |
...
| Visitor access request smoke | Passed for core/security-critical paths | Migration and Cloudflare exception are live. Production smoke confirmed public create, duplicate-pending error, pending list visibility, reject cleanup, deployed direct `?mode=request-access` return UX, attempted public admin role stored as viewer, approve/GitHub invite as viewer, disabled-user approval denial, and cleanup of smoke pending requests. Remaining: rate-limit/readable error polish if found during full sidebar smoke. |
