...
**Current active task (2026-06-02):** full authenticated production sidebar smoke and the Tailscale RTSP real-camera pilot passed; keep frontend work to verified production polish only. Visitor access request production smoke is complete for the security-critical paths, all 10 sidebar pages loaded through `panoptix.site` with no unexpected `404`/`500`/`502`, and `Tailscale RTSP Camera` streamed through the DigitalOcean `dropletGateway`. Smoke observed only transient recovered `401` bootstrap calls and browser-extension console noise; `localStorage`/`sessionStorage` contained no sensitive token material. Do not start Gateway Discovery UI or V380/V380ProQ16S camera integration from the frontend side unless Ivan explicitly reassigns it.
...
6. ~~Smoke every current sidebar page against production data~~ — ✅ Done. Dashboard, Live Cameras, Camera Management, Gateways, Users & Access, Audit Logs, Alerts, System Health, Break Glass, and Settings loaded at `panoptix.site`.
7. Fix only API contract/wiring issues found during later smoke. Do not add new roadmap pages or redesign UI/UX unless explicitly assigned.
8. Do not implement Gateway Discovery UI unless Ivan explicitly reassigns it. Gateway Discovery V2 backend/edge APIs exist and are active on `main`; production-standard on-site camera LAN/VLAN hardening remains system-owner work for future sites.
...
| Implemented and wired | `/api/v1/cameras/{camera_id}/view-token` | ✅ CameraDetailModal uses `@livekit/components-react` as subscriber-only viewer; PR #21 deployed this frontend bundle to production on 2026-05-31 | Production token request smoke passed 2026-05-31 with HTTP 200. Public asset check confirms `index-DudfvjqN.js` is live and old `LiveKit player not wired yet` copy is absent. 2026-06-02 Tailscale RTSP Camera production playback pilot passed through DigitalOcean `dropletGateway`; rerun after new camera/gateway deploys. |
...
| Implemented; production smoke passed | `POST /api/v1/visitor/access-requests` | `/entry` request-access form is present in the deployed production bundle; the public role selector is removed and `/entry?mode=request-access` return UX is deployed | Public creation stores a pending ordinary user/viewer request only. It must not create admin requests, accounts, roles, sessions, camera ACLs, GitHub invites, or Cloudflare authorization. 2026-06-01 smoke: `201 pending` on first submit, `409 access-request-already-pending` on duplicate, manual `requested_role: "admin"` stored `viewer`, approval sent/recorded a GitHub invite as viewer, disabled-user approval returned `user-disabled`, and smoke pending requests were cleaned up. |
...
| Implemented; production smoke passed | `/api/v1/admin/access-requests`, detail, approve, reject | Users & Access has an Access Requests review panel | Production smoke confirmed reject cleanup, approve/GitHub invite as viewer, and `409 user-disabled` when the email belongs to a disabled local user. Continue only with visible UI polish found during later targeted smoke. |
...
| Task | Status | Notes |
|---|---|---|
| Real LiveKit browser viewer playback | ✅ Done | Use backend viewer tokens to connect with the LiveKit client as a subscriber only. Production Tailscale RTSP Camera pilot passed on 2026-06-02. |
...
| Full production smoke test | ✅ Passed 2026-06-01 | All 10 sidebar pages loaded through Cloudflare Access at `panoptix.site`; no unexpected `404`/`500`/`502`; transient recovered `401` bootstrap calls only. |
| Browser publishing absence check | ✅ Passed for browser smoke | Production Live Cameras and Tailscale RTSP pilot smoke showed no camera/mic prompt and no browser publishing behavior; full sidebar smoke did not introduce browser media prompts. |
| Sensitive-value exposure check | ✅ Passed 2026-06-02 | Storage smoke found no sensitive token material in `localStorage` or `sessionStorage`; session storage was empty and auth remained cookie-based. Tailscale RTSP pilot exposed no RTSP URL, gateway token, Cloudflare token, or LiveKit secret in browser storage/logs/docs. |
...
| Visitor access request smoke | Passed for core/security-critical paths | Migration and Cloudflare exception are live. Production smoke confirmed public create, duplicate-pending error, pending list visibility, reject cleanup, deployed direct `?mode=request-access` return UX, attempted public admin role stored as viewer, approve/GitHub invite as viewer, disabled-user approval denial, and cleanup of smoke pending requests. Remaining: rate-limit/readable error polish only if later targeted smoke proves an issue. |
...
Current evidence: Dashboard/bootstrap, live-camera camera list, Users & Access, Camera Management, Gateways, Audit Logs, audit verification, DSR list, break-glass status, backup status, deep health, sessions, and health passed same-origin local smoke against a FastAPI backend using ignored `apps/api/.env` and dev auth. Staging deployed browser smoke passed 2026-05-21. Production expanded visitor collector API smoke passed 2026-05-24. Full 10-sidebar production browser smoke passed 2026-06-01 with no unexpected `404`/`500`/`502`; transient recovered `401` bootstrap calls and browser-extension console noise only; no sensitive token material in `localStorage` or `sessionStorage`. Production Tailscale RTSP Camera playback passed 2026-06-02 through the DigitalOcean `dropletGateway`; RTSP stayed on Tailscale/private networking and browser playback was LiveKit subscriber-only. Treat any one-time gateway service token shown in the UI as sensitive; do not screenshot it.

## Current Default Next Task

Alerts wiring, visitor investigation, actor investigation, LiveKit subscriber playback, audit filtering, visitor access request smoke, full production sidebar smoke, and the Tailscale RTSP real-camera pilot are done. The current next system-owner task is DigitalOcean gateway stability/ops evidence and production-standard on-site gateway/VLAN planning; frontend changes should be limited to verified production polish unless Ivan explicitly reassigns new UI work.
