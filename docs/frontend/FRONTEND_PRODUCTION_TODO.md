...
**Current active task (2026-06-01):** finish visitor access request edge-case smoke and polish. Production migration `0013_visitor_access_requests` is applied, the narrow public `POST /api/v1/visitor/access-requests` Cloudflare exception is active, public create returned `201 pending`, duplicate submit returned `409 access-request-already-pending`, and Users & Access reject cleared the pending smoke request. Current branch hardens public requests to user/viewer-only and adds the `/entry?mode=request-access` return path. Remaining smoke after deploy: attempted public admin role is stored as viewer, approve/GitHub invite with a safe real test identity, disabled-user approval returning readable `user-disabled`, rate-limit/readable error states, then continue full sidebar smoke. Do not start Gateway Discovery UI or V380/V380ProQ16S camera integration from the frontend side unless Ivan explicitly reassigns it.
...
| Implemented; core production smoke passed | `POST /api/v1/visitor/access-requests` | `/entry` request-access form is present in the deployed production bundle; current branch removes the public role selector and adds `/entry?mode=request-access` return UX | Public creation stores a pending ordinary user/viewer request only. It must not create admin requests, accounts, roles, sessions, camera ACLs, GitHub invites, or Cloudflare authorization. 2026-06-01 smoke: `201 pending` on first submit and `409 access-request-already-pending` on duplicate. |
...
| Visitor access request smoke | Core path passed; edge cases remain | Migration and Cloudflare exception are live. Production smoke confirmed public create, duplicate-pending error, pending list visibility, and reject cleanup. Remaining after user-only deploy: attempted admin public request stores viewer, approve/GitHub invite, disabled-user approval denial, direct `?mode=request-access` return UX, and rate-limit/readable error states. |
