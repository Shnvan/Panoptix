...
**Current active task (2026-06-01):** finish visitor access request edge-case smoke and polish. Production migration `0013_visitor_access_requests` is applied, the narrow public `POST /api/v1/visitor/access-requests` Cloudflare exception is active, public create returned `201 pending`, duplicate submit returned `409 access-request-already-pending`, Users & Access reject cleared the first pending smoke request, the deployed frontend includes `/entry?mode=request-access`, and a manual public `requested_role: "admin"` payload was stored as `viewer`. Remaining smoke: approve/GitHub invite with a safe real test identity, disabled-user approval returning readable `user-disabled`, rate-limit/readable error states, then continue full sidebar smoke. Do not start Gateway Discovery UI or V380/V380ProQ16S camera integration from the frontend side unless Ivan explicitly reassigns it.
...
| Implemented; reject smoke passed | `/api/v1/admin/access-requests`, detail, approve, reject | Users & Access has an Access Requests review panel | Browser smoke confirmed pending request visibility and reject clears pending. Remaining: approve invite with a safe real test identity and `409 user-disabled` when the email belongs to a disabled local user. |
...
| Visitor access request smoke | Core path passed; edge cases remain | Migration and Cloudflare exception are live. Production smoke confirmed public create, duplicate-pending error, pending list visibility, reject cleanup, deployed direct `?mode=request-access` return UX, and attempted public admin role stored as viewer. Remaining: approve/GitHub invite, disabled-user approval denial, and rate-limit/readable error states. |
