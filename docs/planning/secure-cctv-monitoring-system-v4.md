# Secure CCTV Web Monitoring System â€” Production Plan (v4, CCTV-only)

A self-contained 29-section production plan that supersedes v3 by removing every webcam / browser-camera / phone-publisher path, promoting the edge gateway from Phase-12 future work to MVP, and applying the Critical/High fixes from the v3 expert diagnosis â€” the system is now strictly **IP camera / NVR â†’ RTSP â†’ edge gateway â†’ authenticated media ingest â†’ LiveKit Cloud (primary) or self-hosted LiveKit (fallback) â†’ authenticated web CCTV dashboard**, with browsers acting only as viewers/admins and never as camera publishers.
...
2. **Always-on managed cloud hosting**: â‰¥2 redundant compute instances. **Laptop / dorm-room / home-PC hosting forbidden at every phase.**
...
8. **Short-lived, kind-distinct stream tokens**: **viewer-subscribe** (browser users, â‰¤60s, subscriber-only, bound to `user_id` + `session_id` + `camera_id`) and **gateway-publish** (edge gateways, â‰¤60s, publisher-only, bound to `gateway_id` + `camera_id` via `gateway_camera_assignments`). A browser session can never receive a gateway-publish token. Both replay-detected via `jti`.
...
10. **No MVP recording**: no server-side recording, no snapshots, no playback. Technical enforcement: no LiveKit Egress, no media bucket, no `MediaRecorder`, no recording UI (CI-gated; Â§24).
...
| Camera ingest | **RTSP-first via `mediamtx` edge gateway**; ONVIF Profile S/T conditional on Â§13.3 spike | Camera credentials live on the gateway; never reach the browser or `cctv-api`. |
...
| Budget | â‰¤ ~US$50/mo MVP, â‰¤ ~US$200/mo pilot (procurement-time) | Free/low-cost where it does not weaken controls. |
...
## 0.1 Revision History (v4 â†’ v4.1)
...
**Numbering convention for new Â§13 subsections**: append-only â€” existing Â§13.4 (Self-hosted LiveKit fallback), Â§13.5 (Hard rules), Â§13.6 (References) keep their content and number. New camera-plane subsections become Â§13.7..Â§13.10. All cross-references to Â§13.5 and Â§13.6 from the rest of the document remain valid.

| Finding | Severity | Where applied | Status |
|---|---|---|---|
| **C-01** Gateway location ambiguity (dev/CI vs production) | Critical | Â§12 stack table; new Â§13.8 Camera Site Hardware; new Â§13.9 Camera Network Design; Phase 4 exit; Phase 0 P0-13; ADR 0013 stub in Â§22 | **Closed** â€” production = on-site NUC-class mini-PC; dev/CI gateway = synthetic only |
| **C-02** `synthetic_rtsp_test_source` undefined | Critical | New Â§13.7 Synthetic RTSP test source; Â§25.1 scaffold reference; Â§25.4 local-dev workflow | **Closed** â€” defined as FFmpeg sidecar (`testsrc` + `sine` â†’ RTSP) |
| **C-03** Break-glass scheduler dependence | Critical | (no edit) | **Reviewed, no change** â€” v4 Â§16.6 already specifies request-time enforcement as authoritative; reviewer misread |
| **C-04** Audit export trust model | Critical | Â§15.1 audit export; Â§17.2 trust-model subsection | **Closed** â€” export bundle includes per-row `prev_hash`/`hash`/`hmac_key_version` + `audit_hmac_keys` snapshot for independent HMAC verification |
| **H-01** CF OTP fails phishing-resistant MFA | High | Â§11.2; Â§22 ADR 0002 description | **Closed** â€” CF OTP dropped from primary IdP options; retained only as IdP-outage fallback |
| **H-02** On-site gateway secret store | High | Â§11.5; Â§20.14 | **Closed** â€” `/etc/cctv-gateway/gateway.env` mode `0600`, owner `cctv-gateway`, systemd `EnvironmentFile=` |
| **H-03** Camera VLAN design | High | New Â§13.9 Camera Network Design; Â§22 ADR 0012 stub | **Closed** â€” dedicated camera VLAN, two-NIC or VLAN-aware single-NIC, outbound 443 only |
| **H-04** `stream_grants` cleanup home | High | Â§14.3; T-50 update | **Closed** â€” scheduled cleanup job every 5 min; external-cron fallback documented |
| **H-05** LiveKit room model | High | New Â§13.10 LiveKit Room Model; `cameras.livekit_room_name` in Â§14.1 | **Closed** â€” one room per camera, presence-driven publish, 10-s grace on last-viewer |
| **H-06** `mediamtx` version pin | High | Â§22 ADR 0007 rename; Â§12 stack table | **Closed** â€” covered by ADR 0007; Renovate watcher for `bluenviron/mediamtx` |
| **H-07** CSP nonce | High | Â§16.5 | **Closed** â€” Next.js-compatible strict CSP and nonce/hash strategy specified |
| **H-08** Webhook replay window + CORS | High | Â§13.5 hard rule 13; Â§15.1 webhook receiver; Â§16.13 CORS; new T-63 | **Closed** â€” 60-s window; webhook empty allow-origin; preflight 405 |
| **H-09** `pg_dump` backup host | High | Â§20.7 | **Closed** â€” scheduled backup job separate from web process |
| **M-01** Bus-factor runbook | Medium | New Â§20.19 | **Closed** |
| **M-02** Camera procurement guidance | Medium | Phase 0 P0-14 | **Closed** |
| **M-03** RBAC expansion path | Medium | Â§16.3 | **Closed** â€” schema supports additional roles without migration |
| **M-04** `pg_dump` integrity (daily vs weekly) | Medium | Â§14.1 `backup_runs`; Â§20.7 | **Closed** â€” `restore_format_ok` (daily) + `restore_schema_ok` (weekly) |
| **M-05** NPC registration in T-46 | Medium | T-46 sub-item | **Closed** |
| **M-06** Viewer-identity watermark at MVP | Medium | New Â§16.19 | **Closed** â€” CSS overlay (deterrence at MVP); video-embedded watermark deferred to pilot |
| **M-07** Telegram alert PII | Medium | Â§16.15; Â§17.4 | **Closed** â€” opaque IDs only |
| **M-08** Dynamic CSP `connect-src` for fallback | Medium | Â§16.5; Â§20.10 cross-ref | **Closed** â€” Middleware reads `system_config.media_plane_mode` per request |
| **M-09** Phase 0 timeline | Medium | Â§21 Phase 0 | **Closed** â€” `5â€“10 business days` with day-by-day sequence |
| **M-10** `nrt` DR scope | Medium | Â§10.4; Â§20.7 | **Closed** â€” `nrt` DR labelled pilot+; MVP DR = backup-restore-to-`sin` only |
| **M-11** `jti` replay scope clarification | Medium | Â§11.4; Â§14.1 `stream_grants`; T-50 | **Closed** â€” distinguishes app token-mint replay (jti) from LiveKit-side defense (60-s TTL) |
| **M-12** `mediamtx` HTTP API auth | Medium | Â§13.8; Â§20.14 | **Closed** â€” bound to `127.0.0.1:9997` only OR disabled; banned on camera VLAN/WAN |
| **M-13** Token-theft anomaly detection | Medium | Â§17.3; new audit event `system.gateway.anomaly.detected` | **Closed** |
| **M-14** Minor consent for school sites | Medium | Â§16.12; T-46 DPA bundle | **Closed** â€” RA 10173 + NPC Circular 16-01 conditional |
| **N-01** SBOM in CI | Addition | Â§20.2 | **Closed** â€” Syft / CycloneDX / sigstore-signed |
| **N-02** CT-log monitoring | Addition | New Â§16.20; Â§20.6 | **Closed** |
| **N-03** Trusted-header policy | Addition | Â§11.6; new T-64 | **Closed** â€” explicit allow-list; deny/ignore forged `cf-access-*` |
| **N-04** Local-dev workflow | Addition | New Â§25.4; Â§25.1 scaffold reference | **Closed** â€” `docker-compose up`; fake-CF-Access middleware gated by `APP_ENV` + `ALLOW_DEV_AUTH=1` |
...
- **What**: A web-based CCTV monitoring console where authorized operators view â‰¥2 live IP-camera feeds. Cameras are reached via an edge gateway running `mediamtx` that pulls RTSP and republishes to LiveKit. **Browsers are viewers only.**
...
| REQ-SEC-03 | Stream URL leak â€” copying URL to another browser denies within â‰¤60 s | T-6 |
| REQ-SEC-04 | Audit append-only + tamper-evident **+ chain integrity across HMAC key rotation** | T-23, T-24, T-31, **T-49** |
| REQ-AV-01 | â‰¥99.5% MVP, â‰¥99.9% pilot | Uptime probes 30 d |
| REQ-QA-01 | OWASP ASVS L2 in-scope met | Â§19 |
| REQ-PRIV-01 | PH DPA (PIA, ROPA, DPO, notice, processor DPAs, retention, **bystander signage**) | Â§16.11 + Â§16.12 |
...
- **IdP**: Google Workspace through Cloudflare Access; Cloudflare OTP is fallback only for IdP outage (Â§11.2).
- **Paid managed Postgres**: Neon-first; Neon Free only for prototype/MVP, Neon paid before pilot if PITR/no-cold-start/DPA/latency checks pass; Supabase Pro fallback (Â§12).
...
- **Bystander population**: assume mixed minors/employees/visitors/public â†’ strict signage policy (Â§16.12).
- **Latency target**: â‰¤2 s glass-to-glass remains the MVP target.
...
- **Continuous streaming**: gateway publishes only while an authorized viewer is present in the LiveKit room (A6â€²); any always-on streaming change is ADR-gated.
...
| **A4** | **Camera source is exclusively IP camera / NVR / RTSP / approved edge gateway (Inv 5)** | Drift to "webcam app" â€” prevented by CI grep (Â§24) and browser-bundle scan |
| A5 | Future ONVIF requires hardware-validation spike (Â§13.3) | Spike surprises |
| A6 | Always-on application (Inv 2) | Demo outage |
| **A6â€²** | **Gateway publishes a camera to LiveKit only while an authorized viewer is present** (room-presence-driven ingress via LiveKit webhooks) | Wrong bandwidth model; surprise bill |
| A7 | No public registration | Account abuse |
| A8 | Control-plane origin invisible (REQ-SEC-01) | Breach |
| A8â€² | Media plane bypasses control plane; if self-hosted, only media ports public (Inv 4) | Broken streaming or coupled blast radius |
| **A8â€³** | **Protected app routes fail closed without verified CF Access JWT (Inv 14)** | Spoofable trusted headers (F-001/F-002) |
...
| A16 | LiveKit quotas verified at procurement; A6â€² keeps usage modest | Throttling |
...
| **A27** | **Edge gateway is MVP-critical (Inv 6)** â€” RTSP-first via `mediamtx`; ships with at least one RTSP ingest before MVP exit | MVP cannot display any camera |
...
Priority: **M**=MVP must, **P**=pilot, **F**=full prod, **X**=out-of-scope MVP, **N**=Not Supported (permanently, see Â§29).

### 5.1 Core viewer & admin

Live view of registered IP cameras â€” **M** Â· Grid 1Ã—1 / 2Ã—1 / 2Ã—2 â€” **M** Â· Fullscreen â€” **M** Â· Camera online/offline/last-seen â€” **M** Â· States: reconnecting, unavailable, permission-denied, gateway-unavailable â€” **M** Â· Watermark viewer identity â€” **P** Â· Snapshots â€” **F** (ADR-gated) Â· Recording / playback â€” **F** (ADR-gated) Â· Motion detection â€” **F** Â· Privacy masking â€” **F**.

### 5.2 Security

- CF Access in front of every authed control-plane route â€” **M**
- Phishing-resistant MFA (WebAuthn/passkeys) via chosen IdP â€” **M**
- Device posture (e.g., WARP) for admins â€” **M**
- App-layer RBAC, deny-by-default â€” **M**
- **Two distinct token kinds** (viewer-subscribe / gateway-publish; browser sessions cannot mint gateway-publish) â€” **M**
- App session bound to CF JWT `sub` + low-risk device fingerprint (UA + Accept-Language only; Â§16.14) â€” **M**
- Strict CSP with nonces; no `unsafe-*`; `connect-src` pinned to specific LiveKit regional hosts â€” **M**
- Full security headers (HSTS preload, COOP/COEP/CORP, Referrer-Policy, **Permissions-Policy locking down camera/microphone**, X-Content-Type-Options) â€” **M**
- CSRF (double-submit + SameSite=Strict + Origin check) â€” **M**
- **Explicit CORS policy** (no wildcard on authenticated APIs; gateway APIs not browser-callable; Â§16.13) â€” **M**
- **Rate limits** (CF + app, Â§16.17 table) â€” **M**
- "CF bot/WAF protections available on the selected plan" â€” **M**
- **Audit hash chain with versioned HMAC keys** + DB-enforced immutability â€” **M**
- Secrets in managed stores; **separate secret store on the media-plane host and on the gateway** â€” **M**
- SCA (Dependabot + osv-scanner), SAST (Semgrep), container scan (Trivy), secret scan (gitleaks) â€” **M**
- IaC scan (tfsec / checkov) â€” **P**
- Pen-test gap log â€” **M**
- **External-exposure test (REQ-SEC-01)** â€” **M**
- **Origin-binding test (REQ-SEC-01c)** â€” **M**
- **Media-plane isolation test (REQ-SEC-01b)** â€” **M**
- **CCTV-only enforcement test set (REQ-CCTV-01)** â€” **M**
- **Break-glass admin** with `BREAK_GLASS_WINDOW_MINUTES = 90` and request-time enforcement â€” **M** (Â§16.6)
- **Lost-MFA recovery** (admin-mediated) â€” **M** (Â§16.7)
- **CF Access rollback runbook** â€” **M** (Â§20.9)
- **LiveKit quota-fallback runbook** with network-layer acceptance â€” **M** (Â§20.10)
- **Gateway lifecycle runbook** (register, rotate credential, disable, retire) â€” **M** (Â§20.14)
- **Provider-exit playbook** â€” **P** (Â§20.13)

### 5.3 Camera & gateway (MVP)

- **Edge gateway running `mediamtx`**, RTSP-first ingest from IP cameras / NVRs â€” **M**
- **Camera registry** (name, source type, assigned gateway, ACL) â€” **M**
- **Gateway registry** (name, identity, status, assigned cameras) â€” **M**
- **`gateway_camera_assignments`** join table (authoritative scope for gateway-publish tokens) â€” **M**
- **Gateway heartbeat** + camera online/offline/last-seen â€” **M**
- **Camera ACL** (per-user, per-camera) â€” **M**
- **Disable / retire camera** â€” **M** (immediate denial of new viewer tokens; existing rooms terminated within 10 s)
- **Disable gateway** â€” **M** (immediate denial of new ingest tokens; existing publish terminated within 10 s)
- **`synthetic_rtsp_test_source`** for development / CI â€” **M** (`mediamtx` test pattern)
- Offline alert â€” **P**
- **ONVIF Profile S/T** â€” **F** (conditional on Â§13.3 spike)
- **Production camera path documented and shipped (RTSP gateway in MVP)** â€” **M**
- **No browser/phone/webcam camera publishing** â€” **N** (Â§29)

### 5.4 User & admin

- List users, role assignment, camera ACL â€” **M**
- Disable user (immediate session kill **+ LiveKit room termination â‰¤10 s** â€” see Â§13.5 hard rule 11) â€” **M**
- Invite via CF Access / IdP â€” **M**
- Audit log view with filter â€” **M**
- **Synchronous signed JSONL export** (no async `export_jobs` queue in MVP â€” see Â§21) â€” **M**

### 5.5 Audit & monitoring

- Append-only `audit_log` + HMAC chain + `hmac_key_version` per row â€” **M**
- All security events (Â§17) â€” **M**
- Daily archive to immutable object storage â€” **P**
- Tamper-check job (5-min cadence) â€” **P**
- **Backup integrity verification** (decrypt + `pg_restore --list` + checksum recorded in `backup_runs`) â€” **M**

### 5.6 Alerting (pilot unless flagged)

Failed-login burst, off-hours admin action, audit chain break, **camera offline >2 min**, **gateway offline >2 min**, error-rate spike, **LiveKit quota threshold**, **CF Access policy change**, **media-plane health degraded**, **gateway certificate expiring â‰¤14 d**, **break-glass opened (M)**, **rate-limit anomaly**, **CSP violation**, **CORS rejection spike**.

### 5.7 Session

Idle 15 min; absolute 8 h; re-auth for admin â€” **M**; revoke-on-disable + LiveKit room termination â€” **M**; "active sessions" UI â€” **P**.

### 5.8 Data & privacy (PH DPA)

- PII minimisation â€” **M**
- Retention: audit 365 d / sessions 30 d / privacy-notice acceptances 7 yrs â€” **M**
- **Operator privacy notice on first login** â€” **M** (Â§16.11)
- **Bystander signage policy & template** at sites â€” **M** (Â§16.12)
- DPA / NPC Circular 16-01 checklist â€” **M** artefacts; **P** full execution
- DSR email channel with 15-day SLA â€” **P**
- **No-Recording-in-MVP policy doc** with technical enforcement â€” **M**
- **Cross-border transfer basis** documented per processor (CF, Railway, paid PG, IdP, LiveKit, R2, Sentry, Better Stack) â€” **M**

### 5.9 Performance / availability

TTFB <300 ms p50 / <800 ms p95 from PH â€” **M** Â· WebRTC glass-to-glass <2 s p95 â€” **M** Â· 99.5% MVP / 99.9% pilot / 99.95% full prod (control plane). Media plane tracked separately.

### 5.10 Operations

Runbooks (deploy, rollback, incident, restore, **CF Access rollback**, **LiveKit quota fallback**, **break-glass**, **lost-MFA**, **IdP outage**, **provider-exit per plane**, **DR to `nrt`**, **gateway lifecycle**, **gateway certificate rotation**) â€” **M / P** as scoped. Daily DB backup + weekly restore drill + **post-backup integrity check** â€” **M**.
...
### 5.12 Permanently Not Supported (see Â§29)

Webcam Â· phone camera Â· laptop camera Â· browser camera Â· browser publisher Â· phone publisher Â· demo publisher Â· lab publisher Â· temporary publisher Â· compatibility publisher Â· `getUserMedia` Â· `MediaRecorder` Â· `navigator.mediaDevices` Â· `/publish` Â· `/demo-publisher` Â· `/lab-publisher` Â· `/webcam` Â· `/phone-publisher` â€” as feature, demo, internal, fallback, test, **and** compatibility paths.
...
|---|---|---|---|---|---|---|
| Live viewing of registered IP cameras | Include | Core | H | M | âœ“ | âœ“ |
| Multi-cam dashboard | Include | Core UX | M | M | âœ“ | âœ“ |
| Fullscreen | Include | Trivial | L | S | âœ“ | âœ“ |
| Camera/gateway status (heartbeat + last-seen) | Include | Trust signal | M | S | âœ“ | âœ“ |
| **Edge gateway (`mediamtx`, RTSP-first)** | **Include â€” MVP** | Only ingest path under Inv 5 | H | M | âœ“ | âœ“ |
| **Camera registry + ACL** | **Include â€” MVP** | Authz/audit | M | S | âœ“ | âœ“ |
| **Gateway registry + assignments** | **Include â€” MVP** | Authz scope for publish tokens | H | S | âœ“ | âœ“ |
| **Synthetic RTSP test source** | **Include â€” MVP** | Dev/CI without hardware | L | S | âœ“ | âœ“ |
| **Camera/gateway health dashboard** | **Include â€” MVP** | Operator trust | M | S | âœ“ | âœ“ |
| Motion detection | Defer | Cost + storage | M | L | âœ— | âœ“ |
| Recording | **Defer; MVP forbids** | Blast radius + DPA | H | L | âœ— | F |
| Playback | Defer | Needs recording | M | M | âœ— | âœ“ |
| Snapshots | F (ADR-gated) | DPA scope | M | S | âœ— | âœ“ |
| RBAC | Include | Mandatory | H | M | âœ“ | âœ“ |
| Audit logs (versioned HMAC) | Include | Mandatory | H | M | âœ“ | âœ“ |
| Login history | Include | Cheap / high value | M | S | âœ“ | âœ“ |
| Session mgmt UI + LiveKit room kill on disable | Include | Real-time revocation (F-003) | H | M | âœ“ | âœ“ |
| Device trust (WARP for admins) | Include | Strong defence | H | S | âœ“ | âœ“ |
| MFA via IdP | Mandatory | Phishing resistance | H | S | âœ“ | âœ“ |
| Admin panel (separate route + policy) | Include | Mandatory | H | M | âœ“ | âœ“ |
| Alerting | Pilot | Needs baseline | M | M | âœ— | âœ“ |
| Tamper detection (5-min) | Pilot | Hash-chain verifier | H | M | âœ— | âœ“ |
| Watermark (viewer email overlay) | Pilot | Deters screen-record exfil | M | S | âœ— | âœ“ |
| Synchronous signed JSONL audit export | Include | Cheap; sufficient at 4-user scale | H | S | âœ“ | âœ“ |
| **Async `export_jobs` queue** | **Drop from MVP** | Premature for 4 users | L | M | âœ— | âœ“ |
| **`webauthn_metadata` mirror** | **Drop from MVP** | IdP holds source of truth | L | S | âœ— | âœ“ |
| **Break-glass admin (90-min fixed window)** | Include | Recovery | H | M | âœ“ | âœ“ |
| **Lost-MFA recovery (admin-mediated)** | Include | Not self-service | H | S | âœ“ | âœ“ |
| **CF Access rollback runbook** | Include | Fast revert | H | S | âœ“ | âœ“ |
| **LiveKit quota-fallback runbook + T-37** | Include | Predictable migrate | H | M | âœ“ | âœ“ |
| **Gateway lifecycle (register / rotate / disable / retire)** | Include | Inv 6 needs ops glue | H | M | âœ“ | âœ“ |
| **Provider-exit playbook (per plane)** | Include | Pricing/quota/security drift | M | M | âœ“ | âœ“ |
| Account lockout via IdP | Include | IdP handles | H | 0 | âœ“ | âœ“ |
| Suspicious login detection | Pilot | CF + app heuristics | H | M | âœ— | âœ“ |
| **Edge gateway certificate rotation (mTLS, before pilot)** | Pilot | ADR 0008 | H | M | âœ— | âœ“ |
| Camera inventory | Include | ACL needs it | L | S | âœ“ | âœ“ |
| Firmware inventory | Future | Real cams only | M | M | âœ— | F |
| NVR integration (`nvr_rtsp` source type) | Pilot | Real customers | M | M | âœ— | âœ“ |
| Multi-site | Future | OOS MVP | M | L | âœ— | F |
| **Webcam / phone / browser publisher (any flavor)** | **Permanently Not Supported** | Inv 5 | â€” | â€” | N | N |
...
- **BreakGlass** account: sealed offline, hardware security key, **`auto_disable_at = opened_at + 90 min` enforced at request time** even if scheduler fails (Â§16.6).
...
- **US-31** Gateway certificate (pilot+) approaches expiry; alert at â‰¤14 d; rotation runbook executes; audit captures issuance/deployment/fingerprint update.
...
1. Cloudflare Access on three policies (App A `/dashboard`, App B `/admin`, App C `/admin-emergency`) in front of the Railway-hosted control plane. **Google Workspace is the primary IdP (Â§11.2).**
...
3. Postgres (Neon free = prototype only; **paid managed Postgres before pilot** per Â§12).
...
9. CI/CD: GH Actions â†’ Trivy + Semgrep + osv-scanner + gitleaks â†’ Railway deploy. **T-30 + T-45 + T-56 + T-58 + T-59 in CI.**
...
- T-37 expanded rehearsed at least once with viewer + gateway reconnection â‰¤60 s after fallback flip.

**MVP launch risks** (quantified Â§23):

- LiveKit quota gap or surprise bill (mitigated by A6â€² + alarms + fallback rehearsed).
...
```
   ...
   ============== CONTROL PLANE (HTTP only, behind CF Access) =========== |
                                      |                                   |
                       DNS (orange-cloud) â†’ CF anycast                    |
                                      |                                   |
       +----------- Cloudflare Edge (HTTP/HTTPS) -----------+             |
       | DNS Â· WAF Â· bot Â· rate-limit (per selected plan)   |             |
       | Cloudflare Access (IAP â€” gateway only, not IdP)    |             |
       |   App A /dashboard /api/v1/{me,cameras,sessions}   |             |
       |   App B /admin /api/v1/admin/*                     |             |
       |   App C /admin-emergency  (break-glass)            |             |
       |   Service-Token policy â†’ /health (monitor probes)  |             |
       | Federates to IdP (Phase 0 Â§11.2)                   |             |
       ...
       |  - mints VIEWER tokens (â‰¤60 s, subscriber-only, jti, session-bd) |
       |  - mints GATEWAY tokens (â‰¤60 s, publisher-only, gateway+camera)  |
   ...
   ============== MEDIA PLANE (WebRTC, NOT through control plane) =======
                                                                       |
        [ LiveKit Cloud (APAC) â€” PRIMARY; UDP preferred, TCP/TLS:443 ] |
                                                                       |
                          --- OR (fallback flag on) ---                |
                                                                       |
        [ Self-hosted LiveKit fallback â€” DigitalOcean SG or equivalent ]|
       ...
       | LiveKit webhook â†’ cctv-api /api/v1/webhooks/livekit (signed)  |

   ============== CAMERA PLANE (RTSP, on-site / VLAN) ==================
                                                                       |
   [ IP camera / NVR ] â†--RTSP--â†’ [ Edge Gateway (`mediamtx`) ]         |
...
- **Cloudflare Access (IAP)** â€” control-plane identity-aware proxy. Federates to the IdP. WAF / bot per selected plan. Issues a **Service-Token policy** for the `/health` endpoint used by external monitors (Better Stack / UptimeRobot).
- **Railway-hosted Next.js frontend** â€” dashboard, admin, emergency, privacy, camera-grid, status, forms, and LiveKit viewer components. It displays state and calls same-origin `/api/v1/*`; it never decides authorization, mints stream tokens, stores long-lived auth tokens, or receives camera RTSP credentials.
- **Railway-hosted FastAPI backend (cctv-api)** â€” API, authz, session validation, viewer-token + gateway-token minting, audit writes, LiveKit room management on user/gateway disable, LiveKit webhook receiver, gateway endpoints, and database access. Origin-bound by fail-closed CF Access JWT verification (Inv 14 / ADR 0010). JWKS cache 10 min, **fail-closed with bounded staleness**. No passwords. No self-service MFA reset. No recording.
- **Postgres** â€” primary store; audit hash-chained with DB-enforced immutability + `hmac_key_version`. **Runtime role least-priv** (Inv 15). Neon free = prototype; paid managed PG with PITR for pilot.
- **LiveKit Cloud (primary media plane)** â€” public TURN/SFU; gateways and viewers connect directly; `iceTransportPolicy: 'relay'` enforced server-side on minted tokens (F-009 fix); 60-second tokens minted by the control plane.
- **Self-hosted LiveKit fallback** â€” separate UDP-capable media host, with DigitalOcean Singapore as the first procurement candidate and equivalent APAC VPS/provider as fallback; only LiveKit media ports + TCP/TLS:443 fallback. **No app endpoints. No DB egress. Separate secret store.** Activated by feature flag after provider verification.
- **Edge gateway (production on-site physical NUC-class mini-PC; dev/CI synthetic host allowed only for fake RTSP)** â€” `mediamtx` ingests RTSP from IP cameras, holds camera credentials in its own secret store, publishes to LiveKit only with a current gateway-publish token, posts heartbeat + camera status to `cctv-api` over CF Access service-token (or mTLS in pilot+). Real cameras are never connected through a virtual/cloud gateway.
- **R2** â€” encrypted archives with object lock (compliance mode); also stores `backup_runs` evidence.
- **Better Stack / Sentry / UptimeRobot** â€” observability with separate tags `plane=control|media|camera`. **Sentry payloads PII-scrubbed** (email, user_id, idp_subject, IP, UA, camera/gateway names hashed; Â§16.15).

### 10.3 Data flows

- **User auth flow**: Browser â†’ CF Access â†’ chosen IdP (challenge + MFA) â†’ IdP redirect â†’ CF mints JWT â†’ Railway-hosted Next.js frontend renders UI; same-origin FastAPI backend verifies CF JWT (`aud`+`iss` pinned, `exp`/`nbf` Â± `CLOCK_SKEW_SECONDS`) on protected API routes.
- **Viewer-token issuance**: viewer requests `GET /api/v1/cameras/:id/view-token` â†’ app authorizes against `camera_acl` AND camera-not-retired AND user-not-disabled AND session-not-revoked â†’ mints LiveKit JWT (â‰¤60 s, subscriber-only, opaque room UUID, `jti` recorded in `stream_grants`) â†’ returns to client.
- **Gateway-ingest issuance**: gateway calls `POST /api/v1/gateways/:id/ingest-token` with service-token / mTLS client cert â†’ app validates gateway identity, gateway-not-disabled, camera-not-retired, `gateway_camera_assignments` row exists and not revoked â†’ mints LiveKit JWT (â‰¤60 s, publisher-only, opaque room UUID, `jti` recorded) â†’ returns to gateway.
- **Media flow (direct, bypasses control plane)**: gateway and viewer use their tokens to connect directly to LiveKit Cloud or fallback. Tokens refresh every â‰¤60 s. **Camera credentials never leave the gateway.**
...
- **Disable-propagation flow**: admin disables user â†’ app revokes session(s) â†’ app calls LiveKit room API to remove participant for that user â†’ audit `auth.session.revoked` + `livekit.participant.removed` (â‰¤10 s SLO; T-49-equivalent integration test in Â§18).
- **Webhook flow**: LiveKit fires room/participant events â†’ `POST /api/v1/webhooks/livekit` (signed; replay-protected by timestamp + HMAC check) â†’ app updates `camera_events` and dashboard SSE stream.

### 10.4 Failure handling & DR

- **CF Access misconfiguration** â†’ rollback runbook (Â§20.9).
- **LiveKit Cloud quota / outage** â†’ fallback runbook (Â§20.10) flips feature flag; viewers + gateway reconnect â‰¤60 s.
- **IdP outage** â†’ CF one-time-PIN secondary policy enabled (Â§20.11).
- **Break-glass** â†’ admin-emergency CF Access app + sealed credential + 90-min auto-disable (Â§16.6).
- **Provider-exit** (CF / Railway / PG / LiveKit) â†’ per-plane migration playbook (Â§20.13).
- **Region/platform outage** â†’ **MVP DR = restore/redeploy to the selected Railway region/environment after provider recovery or to an alternate container-compatible host**. Cross-region hot standby is pilot+ (M-10). Media plane fails over independently to LiveKit Cloud's regional alt or self-hosted instance.
- **Gateway offline** â†’ camera shown as "gateway unavailable"; viewer cannot mint view-token (or token mints but room is empty); alert at 2-minute threshold.
- **Gateway credential compromise** â†’ admin disables gateway â†’ existing publish revoked within 10 s â†’ rotation runbook re-issues credential.
- **DR targets**: RPO â‰¤24h MVP / â‰¤1h pilot (PITR); RTO â‰¤4h MVP / â‰¤1h pilot. Restore-drill runs an integration query post-restore (F-011 fix). MVP RTO assumes restore to the same region; `nrt` cross-region DR (pilot+) tightens this.
...
**Primary IdP MUST support phishing-resistant MFA (WebAuthn/passkey, FIDO2, hardware keys). Email-OTP-only providers are not eligible as the primary IdP** (H-01 â€” RA 10173 + NIST SP 800-63B AAL2 alignment).

| Option | MFA | Fit | DPA | Cost (procurement-time est.) | Pros | Cons | Eligible as **primary**? |
|---|---|---|---|---|---|---|---|
| **Google Workspace** | WebAuthn passkey, hardware keys, push | Common for PH schools | Well-understood DPA | Existing seat; else ~$6/user/mo | Strong MFA | Adds processor | **âœ… Yes** |
| **Microsoft Entra ID** | WebAuthn, hardware keys, Authenticator | If on M365 | Processor DPA available | Free tier available | Strong MFA; conditional access | Complex if not on M365 | **âœ… Yes** |
| **GitHub** | WebAuthn, TOTP | Devs only | Processor DPA available | $0 | Dev-friendly | GH-only identities | **âœ… Yes** |
| **Okta** | WebAuthn, FIDO2, push | Enterprise-ready | Processor DPA available | Paid per user | Most mature | Cost overhead | **âœ… Yes** |
| **CF one-time-PIN** | Email OTP only | Easiest | CF already a processor | $0 incremental | Zero new vendor | **OTP only â€” not phishing-resistant** | **âŒ No** â€” fallback only (Â§20.11) |

**Decision**: Google Workspace is the primary IdP through Cloudflare Access. **Passkey/WebAuthn required**; SMS MFA prohibited (NIST SP 800-63B). **CF one-time-PIN is retained only as the IdP-outage fallback (Â§20.11), where it is constrained, alarmed, and time-boxed.** **Decision artefact: ADR 0002**.

### 11.3 Layered access policies

- **App A** `/dashboard`, `/api/v1/{me,cameras/*,sessions/*}` â€” users group + MFA via IdP.
- **App B** `/admin`, `/api/v1/admin/*` â€” admin group + WARP device posture + recent re-auth â‰¤5 min + (pilot) mTLS client cert.
- **App C** `/admin-emergency` â€” break-glass account only + hardware security key; window auto-disabled at `opened_at + BREAK_GLASS_WINDOW_MINUTES = 90 minutes` (Â§16.6).
- **Service-Token policy** (App D) `/health` â€” single-purpose, monitor-only; rotated quarterly; audit on rotation.
- **Service-Token policy** (App E) `/api/v1/gateways/*` (MVP) â€” gateway-only; one service token per gateway; rotated on credential rotation; replaced by mTLS client cert in pilot+.
...
- **Railway origin-binding (Inv 14 + F-002 fix)** â€” protected routes reject unless the `Cf-Access-Jwt-Assertion` verifies successfully. `Cf-Connecting-IP` and other Cloudflare/forwarding headers are used only as metadata after JWT verification succeeds; otherwise they are ignored/stripped.
...
- Unit + integration tests:
  - **T-47** `nbf = now + 25s` â†’ accepted.
  - **T-48** `nbf = now + 60s` â†’ rejected.
  - Expired token outside tolerance â†’ rejected.
  - Invalid `iss` / `aud` / signature â†’ rejected.
- Semgrep rule `require-cf-jwt-verification` ensures no route skips verification.
- **`jti` replay-protection scope (M-11)**: `jti` is captured in `stream_grants` to prevent replay of the **app's token-mint API call** (a viewer/gateway re-submitting the same mint request to extract another media token). **It is not a media-token replay defense at LiveKit** â€” LiveKit does not consult an external `jti` blocklist. Media-token replay defense is the 60-s TTL: a stolen media token expires before it can be reused at scale. Tests T-50 / T-60 / T-61 verify the right protection at the right layer.

### 11.5 Gateway identity (ADR 0008 â€” Edge Gateway Identity and mTLS CA Design)

**MVP (bootstrap)** â€” strong service-token + pinned fingerprint:
...
- Stored on the gateway as a 0600-mode file:
  - **On-site mini-PC (production, Â§13.8)**: `/etc/cctv-gateway/gateway.env`, mode `0600`, owner `cctv-gateway:cctv-gateway`, loaded via `systemd EnvironmentFile=` directive on the `cctv-gateway.service` unit. Never embedded in code, never logged, never readable by other system users. Rotation procedure in Â§20.14.
...
- Rotation runbook (Â§20.14): generate new token â†’ deliver to gateway â†’ switch over â†’ revoke old â†’ audit `gateway.credential.rotated`.

**Pilot+ (mTLS client cert)** â€” ADR 0008 minimum design:

- **CA**: self-managed internal CA (offline root + online intermediate) seeded during Phase 0; root key escrowed in sealed envelope (dual control). Alternative: Cloudflare mTLS â€” chosen at procurement; ADR 0008 records the decision.
...
- **Fingerprint pinning**: `edge_gateways.mtls_fingerprint` stores the SHA-256 fingerprint of the leaf cert; every gateway request validates: trusted CA chain â†’ certificate fingerprint matches â†’ gateway enabled â†’ request scoped to assigned cameras.
- **Cert expiry tracking**: `edge_gateways.cert_expires_at` stored on issuance.
- **Rotation alert**: alert fires at `cert_expires_at - 14 days`; runbook (Â§20.14) issues a new leaf, deploys to the gateway, updates the fingerprint row, audits issuance + deployment + fingerprint update.
- **Server endpoint verification**: gateway pins the app's TLS leaf or its issuer chain; ADR 0008 records which.
- **Revocation**: compromised gateway credentials â†’ admin disables gateway â†’ fingerprint marked revoked â†’ existing publish terminated within 10 s â†’ rotation runbook issues replacement.
...
**Trusted-header allow-list policy (N-03)** â€” second layer of defense:
...
| Database (prototype) | **Neon Postgres free tier (APAC)** â€” prototype only | Free; fine for early dev | Cold-starts + no PITR disqualify it from production pilot. |
| Database (pilot, pick one) | **Neon Launch / paid equivalent** (~$19/mo, PITR, no cold-starts) primary if checks pass Â· **Supabase Pro** (~$25/mo, PITR) fallback Â· Railway-compatible PG or enterprise PG later if needed | Production needs PITR + no cold-starts + SLA | `sslmode=require`; **least-privilege app role (Inv 15)**; audit table grants `INSERT/SELECT` only. |
...
| Cache | None for MVP | 4 users doesn't justify Redis | â€” |
| Queue | None for MVP | No async jobs (sync JSONL export instead) | â€” |
| Identity (IAP) | Cloudflare Access (see Â§11.2) | Phishing-resistant once federated to passkey IdP | JWT + JWKS verification; `aud` + `iss` pinning. |
...
| **Edge camera gateway** | **`mediamtx vX.Y.Z`** (exact version pinned in ADR 0007 at Phase 0 exit; Renovate watcher tracks `bluenviron/mediamtx` releases) in camera VLAN â€” **RTSP-first**; ONVIF after spike (Â§13.3) | Mature; single Go binary | Read-only FS; outbound-only auth to control plane; no inbound RTSP from internet; HTTP API bound to `127.0.0.1:9997` only OR disabled (Â§13.8 / Â§20.14). |
...
| Hosting (edge gateway â€” **dev / CI / synthetic only**) | Chosen dev/CI host, FFmpeg synthetic-RTSP source (Â§13.7) | Consistent tests; no real cameras | Outbound-only auth to `cctv-api`; LiveKit publish; **never connected to real cameras**; CI artefacts only. |
| **Hosting (edge gateway â€” production)** | **On-site NUC-class mini-PC**, Ubuntu 22.04 LTS x86_64, single Docker image; per-site box (Â§13.8); ADR 0013 | Co-located with cameras; LAN-side RTSP; survives WAN flap | Outbound-only auth to `cctv-api`; LiveKit publish; RTSP pull from camera VLAN only; no inbound WAN ports; secret store at `/etc/cctv-gateway/gateway.env` 0600 + systemd EnvironmentFile (Â§11.5). |
| CI/CD | GitHub Actions â†’ Railway deploy for frontend and backend services | Repeatable deploys; Railway secrets/environment managed per env. | Branch protection; signed commits; lockfile-only installs; frontend bundle scan; Cosign image signing where containers are used. |
...
| Monitoring | Better Stack + UptimeRobot first; Sentry optional when app error tracking is needed | Generous free tiers (verify); paid for pilot | Plane-tagged; **PII scrubbed in Sentry payloads (Â§16.15)**; email-first alerts; Telegram optional with token inventory + rotation runbook (Â§20.16). |
| Logging | App â†’ stdout â†’ Railway logs / Better Stack | Centralized; queryable | Audit logs remain in Postgres (authoritative). |
...
| Backup | GitHub Actions scheduled job: `pg_dump` â†’ `age` encrypt â†’ R2 (object lock); **post-backup `pg_restore --list` integrity check** + checksum + row-count estimate recorded in `backup_runs`; alert on failure | Reproducible, immutable, **verified** | Keys in GitHub Actions backup-job secrets or selected scheduler secret store; weekly restore drill. |
| Incident response | Email-first (MVP); Telegram optional; at least two named responders before pilot | Simple, free | Runbooks in repo; **Telegram bot token rotation runbook + leak-response runbook (Â§20.16)** if Telegram is enabled. |

### 12.1 Cost sketch (monthly, USD, procurement-time estimates â€” verify before committing)
...
| Managed Postgres | $0 (Neon free, prototype) | **~$19â€“30 (paid; mandatory for pilot)** | ~$60â€“100 |
| LiveKit Cloud (A6â€² â€” gateway publishes only when viewer present) | verify quota; likely free for MVP | possibly free or low paid; budget ~$0â€“50 | depends on usage |
...
| Plane | Primary | Realistic alternates if exit triggered (Â§20.13) | Migration handle |
|---|---|---|---|
...
| Media plane | LiveKit Cloud â†’ self-hosted LiveKit fallback | mediamtx WebRTC; another SFU vendor | Feature-flag swap; client SDK changes if vendor changed |
...
|---|---|---|---|---|---|
| **WebRTC SFU â€” LiveKit Cloud** (direct, bypasses CF) | 100â€“500 ms | Token leakage if TTL too long | M | **âœ… Primary** | âœ… |
| WebRTC SFU â€” self-hosted LiveKit on DigitalOcean Singapore or equivalent UDP-capable APAC host | 100â€“500 ms | Same + ops burden | H | **âœ… Fallback after host verification** | âœ… at scale |
| **`mediamtx` RTSPâ†’WebRTC at edge** | 100â€“500 ms | RTSP cred handling on gateway only | M | **âœ… MVP edge gateway** | âœ“ |
| HLS / LL-HLS | 2â€“30 s | Segment URL leakage if unsigned | M | âœ— too slow | âœ“ for recording (future) |
| RTSP relay to browser | â€” | Browsers unsupported | â€” | âœ— | âœ— |
| **Browser publisher (`getUserMedia`)** | â€” | **Forbidden by Inv 5** | â€” | **N** | **N** |

### 13.2 Media-usage model (revised â€” A6â€²)
...
2. **Two distinct token kinds**: viewer-subscribe (â‰¤60 s, subscriber-only) and gateway-publish (â‰¤60 s, publisher-only). Browser sessions can never mint gateway-publish.
...
11. **User disable propagates to LiveKit â‰¤10 s**: the app calls LiveKit room API to remove the disabled user's participant; gateway disable similarly terminates the gateway's publish (F-003 fix).
12. **No browser route may request camera permission, call `getUserMedia`, call `MediaRecorder`, or publish media** (Inv 5).
13. **LiveKit webhook receiver authenticates with shared secret + replay-protected timestamp** (F-012 fix). **Timestamp window = 60 seconds**: webhooks where `abs(now âˆ’ webhook.createdAt) > 60s` are rejected with HTTP 400 and audit `livekit.webhook.replay_rejected`. Webhook is server-to-server only â€” see CORS rules in Â§16.13. T-63 verifies.
...
- **Deterministic**: same FFmpeg flags â†’ same SMPTE-style colour bars + 1 kHz tone; reproducible CI artefacts.
...
- **CI gate**: Phase 4 exit asserts an authenticated viewer can subscribe to `synthetic_cam_01` over LiveKit Cloud and decode the test pattern; latency probe < 2 s p95 (Â§21 Phase 3 / 4).
- **Schema enforcement**: `cameras.source_type = 'synthetic_rtsp_test_source'` is the only enum value the dev/CI gateway is authorized to publish (Â§14.4).
- **Production guard**: a check in the gateway boot script refuses to start the FFmpeg sidecar when the gateway's identity record indicates a production site (Â§13.8). Prevents accidental synthetic publishes at real sites.

Referenced from Â§25.1 scaffold (`docker-compose.yml`) and Â§25.4 local-development workflow.

### 13.8 Camera Site Hardware (production) (C-01)

Production sites run the edge gateway on **on-site hardware**, not on Railway or any cloud app. The dev/CI gateway is synthetic only (Â§12 stack table). ADR 0013 records the standardized hardware class.

**Standard SKU class â€” NUC-class mini-PC (x86_64)**:
...
| CPU | x86_64 quad-core, AES-NI | Intel N100 / N305 or Ryzen 5xxx U-series | Single arch â€” single Docker image |
| RAM | 8 GB | 16 GB | Sufficient for `mediamtx` + agent + headroom |
| Storage | 128 GB SSD | 256 GB NVMe | OS + Docker; no recording on box |
| NIC | 1 Ã— GbE | 2 Ã— GbE | Two-NIC option preferred for camera-VLAN isolation (Â§13.9) |
...
- **`mediamtx`** version pinned (Â§12 stack table; ADR 0007).
...
- **Secrets** at `/etc/cctv-gateway/gateway.env` mode `0600` owned by `cctv-gateway`, loaded via `EnvironmentFile=` (Â§11.5).
...
- **No inbound WAN ports** â€” zero. All app traffic is outbound (CF Access service-token, gateway control WebSocket/heartbeat fallback, and LiveKit publish).
...
|---|---|---|---|
| Outbound (gateway â†’ WAN) | gateway WAN iface | LiveKit Cloud, `cctv-api`, CF Access endpoints, `archive.ubuntu.com`, `*.docker.io` (for image pulls) | **TCP 443 only** |
| Inbound (WAN â†’ gateway) | any | gateway | **None** â€” no port forwards, no NAT loopback |
| Inbound (camera VLAN â†’ gateway) | cameras | gateway camera-side iface | **TCP 554 (RTSP) + ICMP** only |
| Outbound (gateway â†’ camera VLAN) | gateway camera-side iface | cameras | TCP 554 + ICMP only; no DNS, no HTTP camera-config UI traffic |
| Camera VLAN â†” operator LAN | n/a | n/a | **Blocked at switch** â€” cameras cannot see operator workstations |
| Camera VLAN â†’ internet | cameras | any | **Blocked** â€” cameras have no internet route |

**Camera credential handling**: each camera's RTSP password is stored only in the gateway's `mediamtx.yml` (mode `0600`), loaded via the gateway's secret store (Â§11.5). Never reaches `cctv-api`, never reaches the browser, never written to logs (T-62 verifies).

**Operational checklist (site bring-up)**:

1. Inventory cameras: model, serial, RTSP path, ONVIF profile (Phase 4 â†’ Phase 12 if ONVIF).
...
6. Run `mediamtx` config test â€” RTSP pull succeeds.
7. Bind-test: confirm `mediamtx` HTTP API not reachable from camera VLAN or WAN (M-12).
8. Enrol gateway via admin UI (Â§20.14).
9. Verify viewer can subscribe end-to-end with < 2 s p95 latency.
10. Bystander signage attested (Â§16.12).

### 13.10 LiveKit Room Model (H-05)

LiveKit room topology and presence-driven publish (A6â€²).
...
- Stored on the row as `cameras.livekit_room_name VARCHAR(32) NOT NULL UNIQUE` (Â§14.1).
- Room names are opaque from a viewer's perspective â€” they are not the camera's display name and reveal no operator-meaningful information (Inv 5; Â§13.5 hard rule 7).
- A camera deleted (retired) keeps its row + room name forever (audit anchor); a new camera mints a fresh UUID.

**Presence-driven publish flow (A6â€²)**:

1. **Gateway control channel online** â†’ each enabled gateway opens an outbound TLS WebSocket to `cctv-api` after authenticating with its service token (MVP) or mTLS client certificate (pilot+). The channel carries signed command envelopes only; it does not expose an inbound gateway API. If the WebSocket is unavailable, the gateway falls back to heartbeat/polling command pickup on its existing outbound HTTPS heartbeat.
2. **Viewer joins** â†’ viewer-subscribe token minted by `cctv-api` for `room = cameras.livekit_room_name`, scoped subscriber-only, TTL 60 s. Viewer SDK joins the room.
3. **First-participant webhook** (`participant_joined` from LiveKit) hits `/api/v1/webhooks/livekit` with shared-secret HMAC + 60-s timestamp window (Â§13.5 rule 13). `cctv-api` checks: is the room currently publishing? If no â†’ mint a **gateway-publish** token for the gateway assigned to that camera (via `gateway_camera_assignments`), TTL 60 s, scoped to that one room, publisher-only.
...
5. Subsequent viewers receive the existing live track â€” no second publish, no extra LiveKit minutes.
6. **Last-participant-left webhook** (`participant_left`, `participant_count == 0`) â†’ `cctv-api` starts a **10-second grace timer** (handles brief refreshes / reconnects). On grace expiry, `cctv-api` sends `gateway.command.stop_publish` over the outbound WebSocket; the heartbeat fallback carries the same command if the WebSocket is down.
...
- **Webhook lost** â†’ `cctv-api` reconciliation loop polls LiveKit room state every 30 s and corrects drift (terminate orphan publish, restart on missed first-viewer).
- **Gateway WebSocket down** â†’ gateway status becomes degraded after missed ping/heartbeat threshold; pending start/stop commands are delivered via heartbeat fallback. If neither channel is available, camera state becomes gateway-unavailable and token minting fails closed.
- **Gateway slow to stop** â†’ LiveKit-side participant timeout still cleans up the publisher; alert if grace + LiveKit timeout both elapse without a stop ack.
- **User disabled mid-watch** â†’ Â§13.5 rule 11: LiveKit participant removed â‰¤ 10 s; the participant_left webhook then triggers the grace+stop flow if they were the last viewer.
...
- `sites(id, name, address, bystander_signage_attested_at)` â€” site address + signage attestation timestamp (PH DPA Â§16.12).
- **`cameras(id, name, source_type, room_uuid, livekit_room_name VARCHAR(32) NOT NULL UNIQUE, gateway_id, site_id, created_at, retired_at)`** where `source_type ENUM('rtsp','nvr_rtsp','onvif_profile_s','onvif_profile_t','synthetic_rtsp_test_source')`. **`'phone'`, `'webcam'`, `'browser'`, `'browser_publisher'`, `'user_device'`, `'mobile_camera'` are explicitly not part of the enum** and a CHECK constraint enforces it. ONVIF values usable only after Â§13.3 spike passes. **`livekit_room_name`** = `camera_<short_uuid_8>` (first 8 hex chars of `id`); referenced from Â§13.10 LiveKit Room Model.
...
- **`gateway_camera_assignments(gateway_id, camera_id, granted_by, granted_at, revoked_at)`** with composite PK `(gateway_id, camera_id, granted_at)` and partial unique index for one active assignment per `(gateway_id, camera_id)` where `revoked_at IS NULL` â€” authoritative scope for gateway-publish tokens.
...
- **`stream_grants(id, user_id NULL, gateway_id NULL, session_id NULL REFERENCES sessions(id), camera_id, jti, kind ENUM('viewer_subscribe','gateway_publish'), issued_at, expires_at, denied_reason NULL)`** â€” **scope (M-11)**: replay detection for the **app's token-mint API call** (a viewer/gateway re-submitting the same mint request). **Not** a media-token replay defense at LiveKit â€” LiveKit does not consult an external `jti` blocklist; media-token replay defense is the 60-s TTL (Â§11.4). CHECK: `(kind = 'viewer_subscribe' AND user_id IS NOT NULL AND session_id IS NOT NULL AND gateway_id IS NULL) OR (kind = 'gateway_publish' AND gateway_id IS NOT NULL AND user_id IS NULL AND session_id IS NULL)`. **Denied issuance attempts are logged to `audit_log` and recorded with `denied_reason` (no row issued)**.
- **`audit_log(id, ts, actor_id, actor_type ENUM('user','gateway','system','break_glass','service_token_monitor'), action, resource, ip NULL, ua NULL, prev_hash, hash, hmac_key_version INTEGER NOT NULL, payload jsonb)`** â€” append-only via DB triggers; HMAC chain.
- **`audit_hmac_keys(version INTEGER PRIMARY KEY, key_enc bytea NOT NULL, created_at, retired_at NULL)`** â€” encrypted-at-rest key material; the verifier loads the row matching each audit row's `hmac_key_version`.
- `system_config(key, value, updated_by, updated_at)`.
- `incident_reports(id, ...)` (pilot+).
- `break_glass_usage(id, opened_at, opened_by_reason, closed_at, closed_reason, auto_disable_at, rotation_completed_at)` â€” `auto_disable_at = opened_at + interval '90 minutes'` enforced at request time (Â§16.6).
- `privacy_notice_acceptances(user_id, notice_version, accepted_at)` with composite PK `(user_id, notice_version)` â€” IP **not duplicated** here; the `sessions` and `audit_log` rows around the acceptance carry IP if needed (your Â§10 review). Documented in ROPA.
...
- Retention jobs: audit 365 d archive-then-prune (segmented chain across boundary; F-005 fix); sessions 30 d; soft-disable revokes immediately + LiveKit room termination â‰¤10 s.
- **Backups**: nightly `pg_dump` â†’ `age` encrypted â†’ R2 (object lock); **post-backup `pg_restore --list` decrypted-validity check** + checksum + row-count estimate stored in `backup_runs.restore_format_ok` (M-04); weekly schema-restore drill populates `backup_runs.restore_schema_ok`; alert on either being `false` or stale (Â§20.7).
...
CI grep (Â§24) and a schema/migration lint ensure no application code references those forbidden source-type strings outside the Â§29 documentation appendix.
...
- All protected routes verify `Cf-Access-Jwt-Assertion` (`aud`+`iss` pinned, `exp`/`nbf` Â± `CLOCK_SKEW_SECONDS = 30`) and fail closed without a valid CF Access JWT (Inv 14, F-002).
...
- **Explicit CORS policy** (Â§16.13): same-origin only; no wildcard `Access-Control-Allow-Origin` on authenticated APIs; gateway APIs are **not browser-callable** by design.
- **Rate limits** per Â§16.17 table.
...
| **GET** | **`/api/v1/cameras/:id/view-token`** | **viewer with ACL** | **Mint short-lived viewer-subscribe LiveKit token** (â‰¤60 s, subscriber-only, bound to `user_id` + `session_id` + `camera_id`; `jti` recorded; `iceTransportPolicy: 'relay'` enforced). Chooses Cloud or fallback URL based on feature flag. |
...
| POST | `/api/v1/sessions/revoke` | self / admin | Revoke session; triggers LiveKit participant removal â‰¤10 s. |
| **POST** | **`/api/v1/gateways/:id/ingest-token`** | **gateway-authenticated** (service token MVP / mTLS pilot+) | **Mint short-lived gateway-publish LiveKit token** (â‰¤60 s, publisher-only, bound to `gateway_id` + `camera_id` via `gateway_camera_assignments`; `jti` recorded). **Rejected with 401/403 + audit if requester is a browser session.** |
...
| **POST** | **`/api/v1/webhooks/livekit`** | shared-secret HMAC + 60-s timestamp window (Â§13.5 hard rule 13); CORS empty Allow-Origin + preflight 405 (Â§16.13) | Receives LiveKit room/participant events; updates `camera_events`; instructs the gateway to start/stop publishing per A6â€². Server-to-server only â€” not browser-callable. T-63 verifies stale-timestamp rejection. |
| GET | `/api/v1/admin/users` | admin | List users. |
| POST | `/api/v1/admin/users/:id/role` | admin | Assign role. |
| POST | `/api/v1/admin/users/:id/disable` | admin | Disable user **+** revoke sessions **+** call LiveKit room API to remove user's participant â‰¤10 s (F-003 fix). |
...
| POST | `/api/v1/admin/cameras/:id/disable` | admin | Disable/retire camera + terminate viewer rooms â‰¤10 s. |
...
| POST | `/api/v1/admin/gateways/:id/disable` | admin | Disable gateway + terminate publish â‰¤10 s. |
...
| POST | `/api/v1/admin/sites/:id/signage-attest` | admin | Records bystander signage posted at site (Â§16.12). |
| POST | `/api/v1/admin/livekit/fallback` | SuperAdmin | Flip feature flag (control surface for Â§20.10). |

### 15.2 Removed routes (Inv 5)

Permanently absent â€” no code, no route handler, no test path:
...
|---|---|---|---|
| `GET /api/v1/cameras/:id/view-token` | Browser session (CF JWT) | user not disabled Â· session not revoked Â· `camera_acl(user_id, camera_id)` row Â· camera not retired | viewer-subscribe (â‰¤60 s, subscriber-only) |
| `POST /api/v1/gateways/:id/ingest-token` | Gateway (service-token MVP / mTLS pilot+) | gateway not disabled Â· gateway identity matches `:id` Â· `gateway_camera_assignments(gateway_id, camera_id)` row Â· camera not retired | gateway-publish (â‰¤60 s, publisher-only) |
...
- **External attacker** â†’ IAP denies at edge (T-30); protected routes reject direct Railway-origin access without valid CF JWT (Inv 3, Inv 14, T-56); no browser-publisher attack surface (Inv 5).
- **Account compromise** â†’ phishing-resistant MFA (passkey); short sessions; anomaly alert; admin re-auth; break-glass boxed at 90 min.
- **Endpoint compromise of admin device** â†’ WARP device posture; session TTL; admin-panel-only risky actions; sensitive ops require re-auth.
- **Insider (viewer)** â†’ viewer-subscribe tokens only; cannot mint gateway-publish; watermark; full audit.
- **Insider (admin)** â†’ SoD (SuperAdmin required for delete-admin); audit chain with versioned HMAC; break-glass requires rotation on close.
- **Network** â†’ TLS 1.2+; HSTS preload; DNSSEC; CF anycast bot/WAF per plan; direct media networking via LiveKit (no Tunnel routing media); `iceTransportPolicy: 'relay'` prevents host-IP leakage.
- **Media plane compromise** â†’ separate media host, separate secrets, no DB egress, only media ports public (T-45).
- **Origin-bypass attempt** â†’ direct Railway-origin request fails closed without verified CF Access JWT; trusted headers ignored on unverified requests (T-56; F-001, F-002).
- **Gateway impersonation** â†’ service-token hash pinning (MVP), mTLS fingerprint pinning (pilot+); `gateway_camera_assignments` is the only scope; credential rotation runbook (Â§20.14).
- **Camera credential exfiltration** â†’ credentials only on the gateway; never returned from app APIs; CI grep on browser bundle (T-62).
- **Supply chain** â†’ pinned deps, Dependabot, osv-scanner, Trivy, gitleaks, Cosign-signed images.
- **DoS** â†’ CF + app-layer rate limits (Â§16.17); cost alarms; fallback runbook.
- **Webhook spoofing** â†’ shared-secret HMAC + timestamp window + replay cache (F-012).

### 16.2 Authentication

- CF Access JWT required on every protected route (Â§11.4); F-002 trusted-header policy (Â§11.6).
- App session = signed cookie bound to CF JWT `sub` + `session_id` + low-risk device fingerprint (Â§16.14); idle 15 min; absolute 8 h.
- MFA enforced at IdP; WebAuthn/passkey preferred; SMS prohibited.
- No password fields anywhere in the app.

### 16.3 Authorization

- Policy module: `(actor, action, resource) â†’ allow|deny`; deny-by-default.
- Per-request context: actor identity, resolved roles, camera ACL intersection, gateway assignments, session validity.
- Admin mutations: re-auth â‰¤5 min required + audit + (pilot) mTLS client cert.
...
- **Two-tier RBAC (`viewer` / `admin`) is intentional for MVP (M-03)**. Schema (`roles`, `permissions`, `role_permissions`, `user_roles`) supports adding `site_admin`, `camera_manager`, `auditor`, or per-site role variants **without migration** â€” only seed-data changes and a policy-module update. Pilot evaluates whether finer-grained roles are needed; SuperAdmin (delete-admin authority) is a separate flag, not a role row.

### 16.4 Session

- Cookie: `Secure`, `HttpOnly`, `SameSite=Strict`, host-only, short path.
- Server stores `sessions`; on disable: delete session + terminate LiveKit participant â‰¤10 s.
- Idle 15 min, absolute 8 h; admin re-auth â‰¤5 min.
...
- **`Permissions-Policy: camera=(), microphone=(), geolocation=(), autoplay=(self), display-capture=()`** â€” camera and microphone explicitly denied site-wide (Inv 5 defence in depth).
...
- **No `'unsafe-inline'` fallback** even temporarily â€” CI fails the build if the response CSP header contains `'unsafe-inline'` for `script-src` or `style-src`.
...
- **Both** the LiveKit Cloud regional host(s) **and** the self-hosted fallback host (`wss://livekit.<app-domain>`) are pre-approved values **in code** â€” the Middleware chooses which to emit; it never accepts a runtime-supplied origin.
- Switching from Cloud to fallback is a DB flag flip via `POST /api/v1/admin/livekit/fallback` (Â§20.10); no redeploy needed; the next request gets a CSP that allows the fallback origin.
- Cross-reference: Â§20.10 LiveKit fallback runbook describes the operator flow.
...
- **CF Access App C** `/admin-emergency` â€” one allowed identity + hardware-key policy.
- **Window enforcement** (F-006 fix):
  ...
  - **Request-time authorization check**: every request on App C calls `assertBreakGlassActive(now())` which rejects once `now() >= auto_disable_at`. **This is the authoritative gate** â€” the scheduled disabler is a belt-and-braces measure.
...
- **Close runbook** mandates rotation: audit HMAC key â†’ LiveKit API keys â†’ CF Access service tokens â†’ gateway credentials (all).
- **Test (T-52)**: open window, advance clock 91 min (simulated), restart app worker, attempt admin request â†’ must be denied.
...
- **Admin workflow**:
  ...
  3. Invite user to re-enrol during a time-boxed window (â‰¤24 h).
...
- **Telegram bot token** (Â§20.16) â€” rotation runbook + leak-response runbook; documented in secret inventory.
...
- **T-30 external-exposure checklist**:
  ...
  2. CF challenge â†’ 1111-series for unauth.
  3. Cert-transparency leak â†’ only CF-issued certs.
  4. Subdomain enum â†’ no origin leaks.
  5. Shodan/Censys lookups show no unsupported origin endpoint as the documented user entry point.
  6. Zone-transfer attempt â†’ refused.
- **T-45 media-plane isolation**:
  1. Media host public endpoints scanned â†’ only LiveKit media ports reachable.
  2. HTTP `/`, `/admin`, `/api/*` on media host â†’ connection refused.
  3. DB connection from media host â†’ blocked by network policy.
  4. Media-host secrets store scoped to media-plane only.
- **T-56 origin-binding**:
  1. Direct request to Railway-origin URL without CF Access JWT â†’ rejected.
  2. Forged `Cf-Access-Jwt-Assertion` with invalid signature/audience/issuer â†’ rejected.
  3. `Cf-Connecting-IP` / forwarding headers spoofed without valid JWT â†’ ignored.
  4. Request through Cloudflare Access with valid JWT â†’ accepted.

### 16.11 Privacy â€” operators (PH DPA, RA 10173)
...
- **DSR channel**: `dpa@<domain>` â†’ 15-day initial response SLA.
...
- **Retention**: audit 365 d â†’ archive then prune; sessions 30 d; privacy-notice acceptances 7 yrs.
- **Cross-border transfer basis** recorded per processor (`dpa_artifacts.kind = 'cross_border_transfer_basis'`).

### 16.12 Privacy â€” bystanders (PH DPA + NPC Circular 16-01, F-004 fix)
...
- **Alert**: camera registered to a site without signage attestation in the last 12 months â†’ admin dashboard warning.
- **Minor consent for school sites (M-14)**: sites where minors are likely in frame (schools, daycare, after-school programs, paediatric clinics, youth centres) require, in addition to the standard signage above:
  ...
  - This is a **deploy-blocker** â€” admin UI refuses to enrol a camera at a flagged minor-site without the consent procedure on file.
...
- `/api/v1/gateways/*` endpoints set `Access-Control-Allow-Origin` to an empty/disallow response for browser pre-flights â†’ **gateway APIs are not browser-callable**.
- **Webhook endpoint `/api/v1/webhooks/livekit` (H-08)**: server-to-server only.
  - `Access-Control-Allow-Origin` header is **empty** in responses (`Access-Control-Allow-Origin: ` â€” no value).
...
- **Telegram alert messages (M-07)** â€” same scrub plus stricter rules because Telegram channels are the highest-leak-risk surface:
  ...
  - Alert format example: `âš ï¸ gateway gw_abc12345 offline >2min @ 2026-05-07T04:33Z (sev:high) â†’ runbook Â§20.14`.
...
### 16.16 Gateway identity & mTLS (summary; full spec Â§11.5 / ADR 0008)
...
- Revocation on compromise: admin disables gateway â†’ existing publish terminated â‰¤10 s â†’ rotation runbook re-issues.
...
**MVP posture**: a CSS overlay watermark on every live video tile, providing visible deterrence against off-screen recording and screen-sharing leaks. Not tamper-proof â€” a determined insider can disable it with browser devtools â€” but raises the cost of casual leakage and provides forensic value if a screen recording surfaces.

**Implementation**:

- React component `<ViewerWatermark />` rendered as a sibling to each `<VideoTile />` inside the same positioned container.
- Content: `<email-prefix>@<domain> Â· <UTC ISO timestamp updated every 1 s> Â· cam <short_uuid_8>`. Example: `j.dela.cruz@school.edu.ph Â· 2026-05-07T04:33:21Z Â· cam abc12345`.
...
- **Pilot+ upgrade path** (already in v4 Â§16): server-rendered video-embedded watermark via a media-pipeline overlay (LiveKit Egress + ffmpeg or a transcoder); requires recording infrastructure that is not in MVP. Tracked in pilot backlog.
...
- On detection of a cert outside the allow-list:
  - Alert via Telegram (PII-scrubbed per Â§16.15) and email to admin + DPO.
  - Audit `system.ct.unexpected_cert_detected` with cert serial + issuer + fingerprint.
  - Trigger Â§20.6 CT-incident runbook: validate the cert is legitimate (planned change) or treat as compromise indicator (rotate keys, contact CA, file revocation request, re-issue from CF).
- **CAA record** on the app domain pinned to the expected issuer(s); reviewed during Â§20.6 observability checklist.
- Runbook reference: Â§20.6.
...
The synchronous signed JSONL export at `GET /api/v1/admin/audit/export` (Â§15.1) ships three pieces of evidence:

1. **Signed JSONL** of the requested audit rows. The signature attests that the dump was produced by `cctv-api` at signing time â€” nothing more.
...
- It does **not** prove that the underlying audit rows were not tampered with after their original write â€” only that they read this way at signing time.
...
4. Any mismatch â†’ the row was tampered with at-rest after write, *or* the keys snapshot was tampered with. Either way, **independent** of the export signature â€” a compromised `cctv-api` cannot fake this verification because it does not hold the recipient's own out-of-band knowledge of which key versions existed at export time.

**Pilot+ uplift**: external notary or a separate signing-key store (HSM) for the export signature â€” a stronger guarantee that the export bundle itself was not produced by a compromised app. Recommended before any external-auditor reliance.

### 17.3 Monitoring

- Uptime probes (external) to `/health` with service-token header (CF Access App D).
- Better Stack heartbeat with service-token; Sentry error tracking (PII-scrubbed, Â§16.15) tagged `plane=control|media|camera`.
...
- **Per-`gateway_id` ingest-token mint-rate alert**: rolling 7-day baseline computed from `stream_grants WHERE kind='gateway_publish'`; alert if a gateway's 1-hour mint-rate exceeds **2Ã— baseline** (subject to a minimum-volume floor to avoid noise on quiet gateways).
...
- **New audit event**: `system.gateway.anomaly.detected` with reason (`mint_rate_excess` | `source_ip_change` | `unassigned_camera_attempted`) and the gateway's recent activity window. Surfaces in the admin dashboard â†’ Â§20.14 lifecycle runbook covers response (verify with operator â†’ if unverified, disable + rotate).

### 17.4 Observability secret inventory (Â§20.16 cross-reference)
...
- Rotation schedule + leak-response runbook in Â§20.16.
- Each secret has a named owner and a revocation channel.

**Telegram alert format (M-07)**: see Â§16.15. Alerts use opaque hashes (`gw_<8-hex>`, `cam_<8-hex>`, `user_<8-hex>`) with no raw names, emails, IPs, or addresses. Operator looks up the opaque ID in the admin dashboard server-side.
...
|---|---|---|---|
| T-1 | Unauth on `/dashboard` gets CF Access challenge | E2E / net | â€” |
...
| T-5 | Viewer sees only ACL'd cameras | Unit + E2E | â€” |
| T-6 | Copied stream URL denied in 60 s | Net / WebRTC | TTL expiry |
| T-7 | Admin mutation requires recent re-auth | E2E | â€” |
| T-8 | Role change audited | Int | â€” |
| T-9 | Disable user revokes session â‰¤1 s | Int | â€” |
| T-10 | CSP blocks inline script | E2E | â€” |
| T-11 | CSRF double-submit enforced | E2E | â€” |
| T-12 | Rate limits return 429 on token mint | Load | viewer + gateway |
| T-13 | `camera_acl` respected on API | Unit | â€” |
| T-14 | `gateway_camera_assignments` respected on gateway token mint | Unit | â€” |
...
| T-19 | ZAP baseline clean | CI | â€” |
| T-20 | 7-day uptime â‰¥99% staging | SRE | â€” |
| T-21 | Audit chain verifier end-to-end | Int | â€” |
...
| T-25 | Idle session expires at 15 min | E2E | â€” |
| T-26 | Absolute session expires at 8 h | E2E | â€” |
| T-27 | MFA required at IdP on new device | E2E | depends on IdP |
| T-28 | Admin re-auth required â‰¤5 min | E2E | â€” |
| T-29 | No sensitive data in error messages | Unit | â€” |
| T-30 | External-exposure checklist (REQ-SEC-01) | Net | all items pass |
| T-31 | Chain verifier detects a tampered row | Int | negative |
| T-32 | LiveKit token TTL â‰¤60 s | Unit | â€” |
| T-33 | Viewer/gateway token roles distinct | Unit | scope |
| T-34 | Watermark shown in video (pilot) | E2E | pilot |
| T-35 | p95 latency <2 s glass-to-glass | Load/WebRTC | â€” |
| T-36 | Retention job prunes correctly | Int | â€” |
| T-37 | **LiveKit fallback â€” network-layer expanded**: UDP-preferred, TCP/TLS:443 fallback, viewer + gateway reconnection â‰¤60 s, no plaintext creds on wire | Net / WebRTC | media fallback |
| T-38 | CF Access rollback runbook executes cleanly | SRE | â€” |
| T-39 | LiveKit quota approaching â†’ alert fires | SRE | â€” |
| T-40 | Phase-2.5 architecture checkpoint docs produced | Review | â€” |
| T-41 | Self-hosted LiveKit fallback deploy from cold state | SRE | â€” |
| T-42 | Lost-MFA recovery runbook | SRE | â€” |
| T-43 | **RTSP reconnection test** (camera reboot) â€” gateway resumes publish | WebRTC | â€” |
| T-44 | **RTSP credential-leak grep** on packet captures | Net | no cleartext |
| T-45 | **Media-plane isolation** (REQ-SEC-01b) | Net | â€” |
| T-46 | **DPA artefact bundle complete** (incl. bystander signage attestations + cross-border transfer bases) | Review | â€” |
| **T-47** | **JWT skew: `nbf = now + 25s` accepted** | Unit | CLOCK_SKEW_SECONDS = 30 |
| **T-48** | **JWT skew: `nbf = now + 60s` rejected** | Unit | â€” |
| **T-49** | **HMAC chain verifies across `hmac_key_version` rotation** (chain-continuity row after rotation) | Int | F-005 |
| **T-50** | **`stream_grants` cleanup removes expired rows**; replay check stays fast at realistic volume | Int + Load | â€” |
| **T-51** | **Monitor service-token reaches `/health`**; body is exactly `{"status":"ok"}`; no banner/version/DB info | Int | â€” |
| **T-52** | **Break-glass auto-denies after 91 minutes** even after app worker restart (scheduler-failure simulation) | Int / SRE | F-006 |
| **T-53** | **Rate-limit returns 429 + `Retry-After`** for viewer-token mint and gateway-ingest-token mint | Load | Â§16.17 |
| **T-54** | **CORS cross-origin token request rejected; same-origin succeeds**; gateway API pre-flight denied | E2E | Â§16.13 |
| **T-55** | **Gateway heartbeat / camera-status event updates dashboard SSE** (or polling) within target time | E2E | Â§15.1 |
...
| **T-58** | **CI forbidden-term grep**: `getUserMedia`, `MediaRecorder`, `/publish`, `/demo-publisher`, `/lab-publisher`, `/webcam`, `/phone-publisher` absent from active code (whitelisted only in `/docs/` and Â§29 of plan) | CI | REQ-CCTV-01 |
...
| **T-60** | **Token-mint authorization** (Â§15.3): browser calling gateway ingest â†’ 401/403 + audit `gateway.ingest.denied.browser_session`; gateway calling viewer endpoint â†’ 401/403 + audit `viewer.token.denied.gateway_identity` | Int | REQ-CCTV-01 |
...
| **T-63** | **Stale LiveKit webhook rejected**: webhook with `abs(now âˆ’ webhook.createdAt) > 60s` returns HTTP 400; audit row `livekit.webhook.replay_rejected` is written with the offending timestamp; webhook within window succeeds | Int | H-08; Â§13.5 rule 13; Â§15.1 |
| **T-64** | **Forged `cf-access-jwt-assertion` rejected**: (a) injection via direct Railway-origin request â†’ rejected (re-asserts T-56); (b) injection of a forged extra `cf-access-*` header (e.g., `cf-access-username-override`) â†’ ignored before route handler identity construction per the explicit allow-list (Â§11.6) | Int + Net | N-03; Â§11.6 |
...
**ID collision note**: the v3-review's T-47..T-53 findings are re-slotted to T-56..T-62 in v4 to preserve your Â§25 T-47..T-55 numbering intent. T-63 and T-64 are v4.1 additions.
...
- 7-day staging uptime â‰¥99% on control plane.
...
16. No secrets in source; Railway/media/gateway secrets inventory; Telegram bot token rotation runbook present (Â§20.16).
17. IAM deny-by-default verified on every route.
18. CSP violation reports wired to an alert.
19. Session revocation **+ LiveKit room termination â‰¤10 s** verified.
...
26. **Gateway lifecycle runbook** (Â§20.14) present; **gateway cert rotation runbook** (Â§20.15) present.
27. **Telegram bot token rotation / leak-response runbook** (Â§20.16) present.
28. Provider-exit playbook present per plane (Â§20.13).
...
33. **Bystander signage** attested per site (Â§16.12).
34. **No recording enforcement**: no Egress, no media bucket, no `MediaRecorder`, no snapshot route â€” verified by T-58/T-59 + manual review.
35. **Bus-factor runbook** present (Â§20.19) and quarterly drill scheduled.
36. **CT-log monitoring** active (Â§16.20); CAA records present.
...
- GitHub Actions â†’ Railway deploy; avoid long-lived deploy tokens where Railway/GitHub integration supports it.
- Stages: lint â†’ typecheck â†’ unit â†’ integration (testcontainers) â†’ **browser-bundle scan (T-58 + T-59 + T-62)** â†’ Semgrep â†’ osv-scanner â†’ Trivy â†’ gitleaks â†’ **Docker-base pin check (T-58 extension)** â†’ **SBOM generation (Syft â†’ CycloneDX, sigstore-signed; N-01)** â†’ Playwright â†’ ZAP baseline â†’ build â†’ sign with Cosign â†’ staging deploy â†’ smoke â†’ (manual) prod rolling deploy.
...
- **CT-log monitoring (Â§16.20)** runs as a scheduled job on the selected scheduler (Railway cron/scheduled service, GitHub Actions schedule, or equivalent); alerts surface in the same channels as deploy alerts.
...
- Rollback via previous image tag; rollback runbook (Â§20.8).
...
- Logs â†’ stdout â†’ Railway logs / Better Stack.
- Sentry is optional for MVP and enabled when error tracking is needed; if enabled, payloads are PII-scrubbed (Â§16.15) and tagged `plane=control|media|camera`.
- UptimeRobot pinging `/health` with service-token header (no public `/health`); email alerting first, Telegram optional.
- Dashboards per Â§17.3.
- **CT-log monitoring (Â§16.20)**: hourly poll of crt.sh (or CF CT API) for `<app-domain>` and `*.<app-domain>`; allow-list of expected issuers in `/infra/ct-allowlist.json`; on unexpected cert â†’ Telegram + email alert + audit `system.ct.unexpected_cert_detected` + run Â§20.6 CT-incident sub-runbook (validate planned vs. compromise; rotate, contact CA, file revocation if compromise; re-issue from CF). **CAA record** on the app domain pinned to the expected issuer(s); reviewed during this checklist.
...
- Nightly `pg_dump | age -e -r <recipient> | rclone copy - r2:cctv-backups/...` â€” streamed encrypted upload; private key never present on backup machine.
- **Daily integrity check (M-04)**: same machine fetches the just-uploaded archive, decrypts (with the test recipient pubkey scoped to integrity checks only â€” NOT the production decryption key), runs `pg_restore --list` against the archive to validate it is structurally readable, records sha256 + row-count estimate in `backup_runs.restore_format_ok = true`. Alert on `restore_format_ok = false` or upload failure.
- **Weekly automated schema-restore drill**: same scheduled job restores the latest backup to an ephemeral Postgres/test environment, runs the integration-query (audit-chain verification + camera/gateway row counts â€” F-011), and writes `backup_runs.restore_schema_ok = true`. Alert on `restore_schema_ok = false` or stale (no successful drill in the last 8 days).
- **DR targets**: RPO â‰¤24h MVP / â‰¤1h pilot (PITR); RTO â‰¤4h MVP / â‰¤1h pilot.
...
- Steps: identify last-known-good Terraform revision â†’ `terraform plan` on rollback â†’ **dry-run first** â†’ apply â†’ verify CF Access apps A/B/C/D/E with known-good identities â†’ run T-30 post-change.
...
See Â§16.7. Steps: identify user â†’ verify out-of-band â†’ IdP console MFA reset â†’ user re-enrols within 24 h â†’ audit.
...
- **Per-plane playbooks**:
  - **IAP exit**: reconfigure DNS â†’ deploy alternate IAP (Tailscale/Headscale + nginx + IdP-direct, AWS ALB + Cognito + WAF, or Pomerium) â†’ re-verify T-30.
  ...
  - **Postgres exit**: `pg_dump` â†’ restore on target â†’ swap connection string â†’ monitor.
  - **Media-plane exit**: LiveKit Cloud â†’ self-hosted fallback (existing runbook 20.10) â†’ if long-term, evaluate alternate SFU vendor + client SDK swap (ADR-driven).
  - **Edge-gateway exit**: `mediamtx` â†’ alternate RTSP-WebRTC bridge; per-site re-imaging; gateway credentials re-issued.
...
- **Register**: admin creates `edge_gateways` row via admin UI â†’ app mints one-time service token (MVP) or issues mTLS leaf cert (pilot+) â†’ operator installs on gateway host â†’ gateway calls `/heartbeat` with credential â†’ row marked `enabled`.
- **Assign cameras**: admin adds `gateway_camera_assignments` rows â†’ gateway refreshes its camera list via heartbeat response.
- **Rotate credential** (MVP, on-site mini-PC, H-02):
  1. Admin clicks "rotate token" in UI â†’ server generates new token, computes Argon2id hash, persists hash, returns one-time-download with new raw token.
  ...
  6. Admin clicks "revoke old token" â†’ server marks old hash revoked.
  7. Audit row written: `gateway.credential.rotated` with operator user-id + gateway-id + old-token-fingerprint.
- **Rotate credential** (Pilot+, mTLS): issue new leaf cert from intermediate CA â†’ deploy to gateway â†’ verify handshake â†’ update `mtls_fingerprint` â†’ audit `gateway.cert.deployed` + `gateway.cert.fingerprint.updated` â†’ revoke previous leaf.
- **`mediamtx` HTTP API hardening (M-12)**: as part of every site bring-up and every quarterly review, runbook step verifies that `mediamtx.yml` either (a) sets `api: yes` with `apiAddress: 127.0.0.1:9997` (loopback only) **or** (b) sets `api: no`. Audit `gateway.mediamtx.api_check` records the chosen mode. **Failure mode**: any other binding (camera-VLAN IP, WAN IP, `0.0.0.0`) is treated as a security incident â†’ disable gateway â†’ reconfigure â†’ re-enable.
- **Anomaly response (M-13)**: when `system.gateway.anomaly.detected` fires, the on-call admin opens this runbook â†’ contacts the operator out-of-band to verify recent activity â†’ if unverified, disable + rotate credential â†’ if verified (e.g., legitimate IP change due to ISP swap), update the gateway's allow-listed ASN(s).
- **Disable** (temporary): admin sets `status='disabled'` â†’ ongoing publish terminated â‰¤10 s â†’ gateway cannot mint new ingest tokens â†’ row retained for audit.
- **Retire** (permanent): admin sets `status='retired'` â†’ assignments revoked â†’ credential revoked â†’ audit trail preserved per retention policy.
...
- Steps:
  ...
  3. Gateway restarts `mediamtx` with new cert; old cert retained for â‰¤5 min to avoid mid-call break.
  4. App receives next gateway handshake â†’ updates `mtls_fingerprint` + `cert_expires_at`.
  5. Audit `gateway.cert.issued` + `gateway.cert.deployed` + `gateway.cert.fingerprint.updated`.
  6. Revoke previous leaf in CA.
- Failure mode: if new cert handshake fails, gateway falls back to previous cert until window expires, then publish stops â†’ admin alerted â†’ manual intervention.
...
- **Rotation schedule**:
  - Telegram bot token â€” quarterly (routine) + immediately on suspected leak.
  - Sentry DSN â€” annually + on suspected leak.
  - Better Stack / UptimeRobot â€” annually + on suspected leak.
  - CF Access service tokens â€” quarterly (routine) + on leak.
- **Leak response**:
...
| Production secrets (Railway/control plane) | Railway project environment/secrets | Sealed-envelope copy of recovery procedure; **secrets themselves are NOT escrowed** â€” recovery is via Railway account/project access |
...
| R2 backup bucket | CF account | CF account access â†’ bucket access |
...
**Sealed envelope contents** (kept in a secure physical location â€” e.g., a safe at the Controller's registered office):

- Master recovery instructions document (`docs/runbooks/bus-factor-recovery.md`) â€” step-by-step instructions for the secondary admin.
...
2. Confirms audit â€” photo of the unsealed envelope, audit row `system.bus_factor.envelope.opened` written by the secondary admin via break-glass.
...
### Phase 0 â€” Decisions & procurement (**5â€“10 business days**, M-09)
...
- **P0-08** Decide continuous streaming default (A6â€²: off).
...
- **P0-10** Lock Python/FastAPI/Pydantic/SQLAlchemy/Alembic versions; pin experimental-API exclusions; **pin `mediamtx` exact version** (ADR 0007 â€” framework + binary version pin policy).
- **P0-11** Procurement price sanity check (every cost line in Â§12.1 verified against live provider pricing).
- **P0-12** **Bystander signage policy + template drafted** (ADR 0011-equivalent); site inventory captured.
- **P0-13** **Production gateway hardware procurement (C-01, ADR 0013)**: pick NUC-class mini-PC SKU (Intel NUC / Beelink Mini S / MeLE Quieter / equivalent); confirm AES-NI; assemble Ubuntu 22.04 LTS x86_64 image; document baseline systemd unit + secret-store layout (Â§13.8). One-image-per-arch policy locked.
- **P0-14** **Camera procurement guidance (M-02)**: produce `/docs/procurement/camera-spec.md` listing recommended brands/models with confirmed RTSP Profile S support per datasheet; document that ONVIF-only cameras require the Â§13.3 spike before they can be enrolled in Phase 4; per-site pre-purchase checklist (model, firmware, RTSP path, credential rotation policy).
- **Exit**: all ADRs drafted (0001â€“0013) and open-questions log closed; mini-PC SKU + Ubuntu image baseline locked; camera spec doc landed; processor-DPA templates ready for signing; no blockers to Phase 1.

**Day-by-day sequence (suggested)**:

- **Days 1â€“2**: blocking decisions â€” IdP (P0-01), Postgres tier (P0-02), CF tier (P0-04), LiveKit quota (P0-05), fallback-host feasibility (P0-06).
- **Days 3â€“5**: ADR drafts 0001â€“0014 (one to three per day); P0-09 gateway-identity bootstrap detail; P0-10 framework + `mediamtx` version pin.
- **Days 6â€“8**: DPA template drafting (P0-03), bystander signage template (P0-12), site inventory, NPC registration check (M-05).
- **Days 9â€“10**: hardware procurement decisions (P0-13 mini-PC SKU; P0-14 camera spec doc); ADR 0012 + 0013; provider-DPA signing once templates are agreed.

### Phase 1 â€” Repo, IaC, and CI skeleton (2â€“4 days)
...
- **CI forbidden-term grep** (T-58) + smart whitelist for `/docs/` and Â§29 of plan.
...
### Phase 2 â€” Identity, session, authz (4â€“6 days)
...
- Trusted-header stripping/ignore policy (Â§11.6) for unverified requests.
- Signed cookie + session store + low-risk device fingerprint (Â§16.14).
- Policy module with deny-by-default; unit tests.
- Role/permission tables + seeds.
- **Exit**: T-1..T-4 + T-47 + T-48 + T-56 + T-57 pass in CI.

### Phase 2.5 â€” Architecture checkpoint (1 day)
...
### Phase 3 â€” Viewer flow + audit (4â€“6 days)

- Cameras registry + camera ACL.
- Viewer-subscribe token mint (`/api/v1/cameras/:id/view-token`) with replay protection, TTL â‰¤60 s, ACL checks.
...
### Phase 4 â€” Edge gateway + CCTV ingest (MVP-critical) (5â€“7 days)

- Dev/CI synthetic gateway host (FFmpeg synthetic-RTSP source per Â§13.7) **and** on-site mini-PC Ubuntu image (production; Â§13.8) with `mediamtx` pinned version.
...
- Room-presence-driven gateway publish (A6â€²; Â§13.10 LiveKit Room Model).
- **Exit (revised, C-01)**: **synthetic-RTSP path runs end-to-end in CI** (RTSP â†’ dev/CI synthetic gateway â†’ LiveKit Cloud â†’ authenticated viewer with <2 s p95 latency); T-14 + T-43 + T-60 + T-61 + T-62 + T-55 + T-63 pass. **Production-camera path (real IP camera â†’ on-site mini-PC â†’ LiveKit Cloud)** is validated at the **first real-site cutover** rather than at MVP-exit â€” the on-site box is a procurement artefact, not a CI artefact.

### Phase 5 â€” Admin panel + user ops + DPA (4â€“5 days)

- Admin routes (CF Access App B); re-auth â‰¤5 min.
- User CRUD, role assignment, camera ACL, gateway registration, gateway assignment.
- Disable user â†’ session revoke â†’ LiveKit participant removal â‰¤10 s (F-003).
...
### Phase 6 â€” Security hardening pass (3â€“5 days)
...
### Phase 7 â€” Media-plane fallback pre-build (2â€“3 days)
...
### Phase 8 â€” External exposure, origin-binding, pen-test-self-attack (2 days)
...
- Run full self pen-test checklist (Â§19).
- **Exit**: Â§19 at 100%.

### Phase 9 â€” SRE, observability, backup drill (2â€“3 days)
...
### Phase 10 â€” Compliance, content, and demo (2â€“3 days)
...
### Phase 11 â€” Pilot onboarding (1 day)

- Upgrade Postgres to paid tier (ADR 0003).
- Migrate DNS/custom domain to production Cloudflare Access â†’ Railway path.
...
### Phase 12 â€” ONVIF + optional multi-site (conditional)

- ONVIF hardware spike (Â§13.3).
...
- **Architecture**: write ADRs 0001 (Access gateway), 0002 (IdP â€” phishing-resistant MFA required for primary; CF OTP only as IdP-outage fallback per H-01), 0003 (Paid PG), 0004 (Monolith-vs-split), 0005 (LiveKit fallback), 0006 (Provider-exit criteria), **0007 (Framework + binary version pin policy â€” covers Python/FastAPI/Pydantic/SQLAlchemy, LiveKit client/server SDKs, and **`mediamtx`**, per H-06)**, **0008 (Edge gateway identity + mTLS CA)**, **0009 (CCTV-only invariant)**, **0010 (Origin-binding + trusted-header policy)**, **0011 (Bystander signage policy)**, **0012 (Camera Network Design â€” per-site VLAN topology, firewall rules; H-03)**, **0013 (Gateway hardware standard â€” NUC-class mini-PC, Ubuntu 22.04 LTS x86_64, single Docker image; C-01)**, **0014 (Railway + Python control plane)**.
...
- **Authz**: policy module with unit tests; admin re-auth â‰¤5 min.
...
- **Security**: strict CSP + nonces + `Permissions-Policy`, security headers, CSRF, **rate limits per Â§16.17**, **CORS per Â§16.13**, secrets store, ZAP in CI, **PII scrub in Sentry/logs**.
...
| 4 | Stream URL exfil by insider | M | M | 60-s tokens + kind-distinct scopes + jti replay (token-mint scope) + watermark (Â§16.19) + audit | Low-M |
| 5 | Phishing of an admin | L-M | H | Passkey MFA + WARP + admin re-auth + break-glass 90-min | Low |
| 6 | LiveKit quota hit mid-demo | M | M | A6â€² + alarms + pre-built fallback + T-37 expanded | Low |
...
| 9 | CF Access misconfig locks users out | L | H | IaC + drift detector + rollback runbook (Â§20.9) | Low |
...
| 12 | IdP outage prevents all login | L | H | CF one-time-PIN secondary (Â§20.11) | Low |
...
| 18 | **Hostile cert issued for app domain via different CA** | L | H | CT-log monitoring (Â§16.20, N-02) + CAA records pinned to expected issuers; alert + runbook on unexpected cert | Low |

---

## 24. Edge Cases & Failure Scenarios

- Clock skew on CF JWT â†’ tolerate up to `CLOCK_SKEW_SECONDS = 30`; reject beyond; T-47/T-48.
- JWKS refresh failure â†’ fail-closed with bounded staleness; alert.
- LiveKit outage â†’ activate fallback (Â§20.10); T-37 expanded.
- IdP outage â†’ CF one-time-PIN secondary (Â§20.11).
- Railway app health failure â†’ Railway health checks and external probes alert; rollback/redeploy or alternate-host DR path.
- Postgres outage â†’ read-only mode banner; backup job paused; alert.
- Sentry/Better Stack outage â†’ app continues; audit log authoritative.
- User disable mid-session â†’ revoke + **LiveKit participant removed â‰¤10 s**.
- **Gateway disable mid-publish** â†’ **LiveKit publisher removed â‰¤10 s**; camera shown offline.
- **Gateway heartbeat stale > 2 min** â†’ dashboard shows gateway offline; alert.
- **Camera rejoin after brief RTSP outage** â†’ mediamtx reconnects; gateway resumes publish; camera_events row.
- Admin disables self â†’ rejected with 409.
- Delete last SuperAdmin â†’ rejected with 409.
- Malformed tokens â†’ 401; no info leak.
- Stream URL leaked via screen-sharing â†’ 60-s TTL + watermark + audit.
- Break-glass opens and stays open past 90 min â†’ request-time denial; alert; post-mortem.
- **Self-hosted LiveKit fallback activates but UDP blocked on client network** â†’ TCP/TLS:443 fallback engages; T-37 expanded covers this.
- **mTLS cert expires during live session** â†’ grace period â‰¤5 min while new cert deploys; alert at `cert_expires_at - 14 d`.
- **Webhook replay attack** â†’ HMAC + timestamp window + `webhook_replay_cache` â†’ rejected; audit.
- **Provider pricing/quota change blocks MVP** â†’ provider-exit playbook (Â§20.13) triggers ADR and migration.
- **Compromised monitoring credential (Telegram bot token leaked)** â†’ Â§20.16 leak-response runbook.
- **Floating Docker tag pulls new minor version with regressions** â†’ CI fails; Dependabot PR for pinned update.
- **Operator tries to use a browser camera to "just demo quickly"** â†’ there is no UI or API for this; CI grep + browser-bundle scan enforce it; Inv 5 is product-defining.
...
/cctv/
  /apps/
    ...
    /cctv-edge/           # `mediamtx` container + gateway agent
      mediamtx.yml        # RTSP-first; synthetic_rtsp_test_source in dev (Â§13.7)
      gateway-agent/      # heartbeat + status + token refresh
      Dockerfile          # x86_64-only; single image (Â§13.8)
      deployment.md       # dev/CI synthetic only; production runs on on-site mini-PC
  /docker-compose.yml     # local-dev workflow (Â§25.4): web + api + Postgres + mediamtx + ffmpeg synthetic source + fake-CF-Access middleware
  /infra/
    /terraform/
...
5. `/infra/terraform/` skeleton â€” apply to `staging` first with only the placeholder page.
...
- One fallback rehearsal: flip LiveKit feature flag live; viewers + gateway reconnect â‰¤60 s.
...
**Goal**: a developer can clone the repo, run a single command, and have a working stack on their laptop â€” without exposing any production credential, without bypassing CF Access verification logic, and without a real camera.

**Stack** (defined in `/docker-compose.yml`):

- **`cctv-api`** â€” FastAPI dev server, hot-reload, talks to local Postgres.
- **Postgres 16** (containerized, ephemeral volume).
- **`mediamtx`** + **FFmpeg synthetic-RTSP sidecar** producing `rtsp://mediamtx:8554/synthetic_cam_01` (Â§13.7).
...
- **Fake-CF-Access middleware** â€” a thin dev-only middleware that injects a `cf-access-jwt-assertion` header signed with a **dev-only key** loaded into the app, so the app's normal JWT verifier path runs against a JWT signed by a key it trusts in dev.
...
- **The CF JWT verifier itself is not bypassed** â€” the dev middleware injects a properly-signed JWT with a dev `iss` and dev `aud`; the verifier validates it against the dev signing key. This means the dev environment exercises the same code path as production, just with a different key set. This is the deliberate design (a verifier that is bypassed in dev does not catch real bugs).
...
|---|---|---|---|
| Remove webcam/browser-publish from product | Still referenced as "temporary" | **Removed; Not Supported forever** | Inv 5, Â§5.12, Â§13.5 hard rule 12, Â§15.2, Â§29 |
| Edge gateway in MVP | Phase 12 future | **MVP-critical (Phase 4)** | Inv 6, A27, Â§9, Â§13.3, Â§21 Phase 4 |
| `hmac_key_version` + rotation continuity | Missing | **Added; T-49** | Â§14.1, Â§14.3, Â§17.2, Â§18.2 T-49 |
| `CLOCK_SKEW_SECONDS = 30` as named constant | Env-configurable | **Named constant** | Â§0, Â§11.4, Â§18.2 T-47 + T-48 |
| `BREAK_GLASS_WINDOW_MINUTES = 90` request-time | 4h, scheduler-dependent | **90 min, request-time + external monitor** | Â§16.6, T-52 |
| `stream_grants` two kinds + cleanup + alarms | Single kind | **Viewer-subscribe + gateway-publish; cleanup; alarms** | Â§14.1, Â§14.2, Â§14.3, T-50 |
| `edge_gateways` table with mTLS fingerprint reserved | Absent | **Added; nullable in MVP** | Â§14.1, ADR 0008 |
| `gateway_camera_assignments` authz scope | Absent | **Added** | Â§14.1, Â§16.3, T-61 |
| LiveKit webhook auth + replay | Unsecured receiver | **HMAC + timestamp + replay cache** | Â§13.5 hard rule 13, Â§14.1 `webhook_replay_cache` |
| Rate-limit table | Partial | **Full table** | Â§16.17, T-53 |
| CORS policy (no wildcard) | Partial | **Explicit; gateway APIs not browser-callable** | Â§16.13, T-54 |
| Device fingerprint policy | Undefined (F-007) | **UA + Accept-Language only; mismatch â†’ re-auth** | Â§16.14 |
| PII scrub in Sentry/logs | Undefined | **Sentry `beforeSend`; stdout same rules** | Â§16.15 |
| Permissions-Policy camera=() | Implicit | **Explicit site-wide** | Â§16.5 |
| Health endpoint behind service token | Public | **CF Access service token App D; `{"status":"ok"}` exactly** | Â§15.1, T-51 |
| Deep health admin-only | Absent | **`/api/v1/admin/health/deep`** | Â§15.1 |
| Origin-binding (F-001) | Missing | **Inv 14; T-56** | Â§11.4, Â§11.6, Â§16.10 |
| Trusted-header policy (F-002) | Missing | **Identity only from verified CF Access JWT; trusted headers ignored on unverified requests** | Â§11.6 |
| User-disable â†’ LiveKit kill â‰¤10 s (F-003) | Missing | **Hard rule 11; integration test** | Â§13.5, Â§10.3 |
| Bystander signage (F-004) | Missing | **Â§16.12 + `dpa_artifacts` + attestation endpoint** | Â§16.12 |
| Audit rotation (F-005) | Single key | **Versioned; T-49** | Â§14.1 `audit_hmac_keys`, Â§17.2 |
| Break-glass (F-006) | 4h, scheduler | **90 min, request-time** | Â§16.6 |
| CSP wildcard livekit.cloud (F-008) | Wildcard | **Pinned regional hosts** | Â§16.5, Â§13.5 hard rule 14 |
| `iceTransportPolicy: 'relay'` (F-009) | Client-hinted | **Server-enforced on minted tokens** | Â§13.5 hard rule 10 |
| DB least-priv (F-010) | Partial | **Inv 15; T-57** | Â§14.3 |
| DR RPO/RTO (F-011) | Unstated | **MVP â‰¤24h/â‰¤4h, pilot â‰¤1h/â‰¤1h; restore-drill integration query** | Â§10.4, Â§20.7, T-22 |
| LiveKit webhook auth (F-012) | Unauthenticated | **HMAC + timestamp** | Â§13.5 hard rule 13 |
| Docker base/runtime pinned + Dependabot | Drifted floating tag | **Exact Python patch where containerized; CI fails on floating tag; Dependabot** | Â§12, Â§20.2 |
| `webauthn_metadata` mirror | Present | **Removed from MVP** | Â§5.11, Â§6, Â§14 (absent) |
| `export_jobs` async queue | Present | **Removed from MVP; sync signed JSONL** | Â§5.4, Â§6, Â§15.1 |
| Backup integrity verification | Implicit | **`backup_runs` + `pg_restore --list`** | Â§14.1, Â§20.7 |
| Telegram secret rotation | Missing | **Â§20.16 inventory + rotation + leak-response** | Â§20.16 |
| T-47..T-55 tests per user Â§25 | Absent | **Present at exact IDs** | Â§18.2 |
| CCTV-only enforcement tests | Absent | **T-58, T-59, T-60, T-61, T-62** | Â§18.2 |
| No browser-publisher routes anywhere | Still present | **Removed from Â§15; Â§29 appendix** | Â§15.2, Â§29 |
| UI reflects CCTV-only | Partial | **Admin cameras/gateways pages; viewer has no publish UI** | Â§15.1, Â§25.1 |
| ADRs 0008..0011 | Absent | **Added to Â§22 backlog** | Â§22, Â§21 Phase 0/1 |
...
|---|---|---|
| 1 | No `/publish`, `/demo-publisher`, `/lab-publisher`, `/webcam`, `/phone-publisher` routes in MVP, pilot, or prod | Â§15.2; Â§29; Inv 5 |
| 2 | No `navigator.mediaDevices`, `getUserMedia`, `MediaRecorder`, browser camera permission flow | Inv 5; Â§5.2 `Permissions-Policy: camera=()`; Â§13.5 rule 12; Â§29; T-58; T-59 |
| 3 | Camera ingest via edge gateway authenticated with service token (MVP) / mTLS (pilot+) | Â§11.5; Â§14.1 `edge_gateways`; Â§15.1 `/api/v1/gateways/*`; T-61 |
| 4 | `cctv-api` does not hold camera credentials | A29; Â§13.3; Â§13.5 rule 5; T-62 |
| 5 | `/health` behind CF Access service token; body is `{"status":"ok"}` | Â§11.3 App D; Â§15.1; T-51 |
| 6 | Plan / docs / CI enforce CCTV-only posture | Â§24 forbidden-term grep; Â§16.5 `Permissions-Policy`; Â§25.1 `browser-bundle-scan.sh`; T-58 |
| 7 | Browser-bundle scan of built assets | Â§18.2 T-59; Â§20.2; T-62 |
| 8 | Versioned HMAC + `hmac_key_version` column + rotation procedure | Â§14.1 `audit_log`/`audit_hmac_keys`; Â§17.2; T-49 |
| 9 | `CLOCK_SKEW_SECONDS = 30` named constant | Â§0; Â§11.4; T-47; T-48 |
| 10 | `BREAK_GLASS_WINDOW_MINUTES = 90` + request-time enforcement + external monitor | Â§16.6; T-52 |
| 11 | `stream_grants` with `kind`, composite `(jti, expires_at)` index, cleanup, alarm | Â§14.1; Â§14.2; Â§14.3; T-50 |
| 12 | `edge_gateways.mtls_fingerprint` reserved; MVP ships without it | Â§14.1; Â§11.5 |
| 13 | `audit_log` retention segmented across rotation boundary | Â§14.3; Â§17.2 |
| 14 | Signed LiveKit webhook + replay cache | Â§13.5 rule 13; Â§14.1 `webhook_replay_cache`; Â§15.1 |
| 15 | Rate-limit table for all token-mint and gateway endpoints | Â§16.17; T-53 |
| 16 | CORS policy (no wildcard on authed APIs; gateway APIs not browser-callable) | Â§16.13; T-54 |
| 17 | Device-fingerprint policy documented (UA + Accept-Language only) | Â§16.14 |
| 18 | PII scrub in Sentry + stdout logs | Â§16.15 |
| 19 | `webauthn_metadata` and async `export_jobs` removed from MVP | Â§5.11; Â§6; Â§14 (absent) |
| 20 | ADR 0008 (gateway identity + mTLS), 0009 (CCTV-only), 0010 (origin-binding), 0011 (bystander signage) | Â§22; Â§21 P0-09 + P0-12 |
| 21 | Test matrix covers T-47..T-55 at those IDs | Â§18.2 |
| 22 | Gateway lifecycle runbook, gateway cert rotation runbook, Telegram rotation runbook | Â§20.14, Â§20.15, Â§20.16 |
| 23 | UI reflects CCTV-only: admin pages for cameras/gateways; viewer has no publisher UI | Â§15.1; Â§25.1 |
| 24 | Bystander signage policy + per-site attestation | Â§16.12; Â§14.1 `dpa_artifacts`; Â§15.1 signage-attest endpoint |
| 25 | Cross-border transfer basis per processor | Â§16.11; Â§14.1 `dpa_artifacts` |
| 26 | DR RPO â‰¤24h MVP / â‰¤1h pilot; RTO â‰¤4h MVP / â‰¤1h pilot; post-restore integration query | Â§10.4; Â§20.7; T-22 |
| 27 | Python Docker base/runtime pinned to exact patch where containerized; CI fails on floating tag | Â§12 row "Container"; Â§20.2 |
| 28 | Dependabot (or equivalent) enabled | Â§12 row "Container" / "Monitoring"; Â§20.2 |
| 29 | Demo script is CCTV-only (no browser-publisher steps anywhere) | Â§25.3; Â§21 Phase 10 |
| 30 | **Audit export bundle includes raw HMAC chain + `audit_hmac_keys` snapshot for independent verification** | Â§15.1; Â§17.2 trust-model subsection |
| 31 | **Production gateway is on-site NUC-class mini-PC; dev/CI gateway is synthetic only** | Â§12 stack table; Â§13.8; ADR 0013; Â§21 P0-13; Â§21 Phase 4 exit |
| 32 | **`synthetic_rtsp_test_source` has a defined FFmpeg implementation** | Â§13.7 |
| 33 | **`mediamtx` exact version pinned in ADR 0007 with Renovate watcher for `bluenviron/mediamtx`** | Â§22; Â§12 stack table |
| 34 | **LiveKit webhook timestamp window = 60 seconds; T-63 verifies stale-timestamp rejection** | Â§13.5 rule 13; Â§15.1; T-63 |
| 35 | **On-site gateway secret store specified** (`/etc/cctv-gateway/gateway.env` 0600 + systemd EnvironmentFile) | Â§11.5; Â§20.14 rotation steps |
| 36 | **Trusted-header allow-list documented; T-64 verifies forged-header rejection** | Â§11.6; T-64 |
| 37 | **SBOM (Syft / CycloneDX) generated in CI, sigstore-signed, attached to GitHub releases** | Â§20.2 |
| 38 | **CT-log monitoring active for app domain; CAA records pinned** | Â§16.20; Â§20.6 |

**v4.1 additional acceptance** (closed in this revision):

- 30: **C-04** closed (audit export trust model).
- 31: **C-01** closed (gateway location â€” production hardware locked).
...
- All other v4.1 findings (H-01, H-03, H-04, H-05, H-07, H-09, M-01..M-14) are tracked under criteria 1â€“29 plus the Â§0.1 revision history table for traceability.
...
- This v4 plan file (`secure-cctv-monitoring-system-v4-ef15d0.md`) â€” 29 sections + Â§29 appendix + Â§0.1 revision history (v4.1); CCTV-only posture; diagnosis Critical/High fixes applied; **v4.1 in-place revision closing C-01..C-04 (C-03 reviewed, no change), H-01..H-09, M-01..M-14, N-01..N-04**; acceptance criteria table 1..38 (Â§27); risk register 1..18 (Â§23).
- SUPERSEDED banner on `secure-cctv-monitoring-system-v3-801515.md` (history preserved, path redirected).
- Work-plan file (`cctv-v4-rewrite-workplan-ef15d0.md`) â€” preserved for audit trail.
...
- Application code â€” begins in Phase 1 after ADRs land.
- ADR 0001..0011 full content â€” stubs in Phase 0, full text by Phase 3.
- Runbooks Â§20.8â€“Â§20.16 full content â€” stubbed Phase 1, finalized per phase.
- Terraform modules.
- CI workflow files.
- Test implementations (T-1..T-62) â€” implemented across Phases 2â€“9.
- Dockerfile.
- PIA / ROPA / processor-DPA documents â€” drafted during Phase 10.
- Bystander signage physical posting â€” operations activity, attested via API endpoint in Phase 5.

**Expected final-handoff artefacts at the end of implementation (Phase 10 exit)**, per your Â§28 directive:

1. Summary of changed files (this list grows once implementation begins).
2. Removed features/routes (Â§15.2; Â§29).
3. Added CCTV requirements (Â§5.3; Â§9; Â§13.3).
4. Diagnosis fixes mapped (Â§26 table; F-001..F-012).
5. Schema delta (Â§14).
6. Removed routes (Â§15.2).
7. Added routes (Â§15.1).
8. Added tests (Â§18.2 T-47..T-64).
9. Added CI checks (Â§20.2; Â§24 if code phase).
10. Any remaining CI/test commands required (none for this plan-only hand-off).
11. Any residual webcam mentions â†’ expected **only** in Â§29 (verified by Â§29 self-check grep below).
12. Remaining risks / TODOs (Â§23).
...
<!-- cctv-allow-forbidden-terms: BEGIN (Â§29 Not Supported) -->
...
`cameras.source_type` does not accept values representing these sources; a CHECK constraint enforces it (Â§14.4). CI grep (Â§24) blocks any code path that implies these sources.

### 29.2 Permanently removed publishing paths

- **browser publisher** â€” any UI or API that uploads media from a browser as a camera feed.
- **phone publisher** â€” mobile-browser version of the same.
- **demo publisher** â€” a "for the demo only" browser publisher.
- **lab publisher** â€” an "internal testing" browser publisher.
- **temporary publisher** â€” a "we'll remove it before pilot" publisher.
- **compatibility publisher** â€” a "for browsers that don't have the SDK" publisher.
...
`Permissions-Policy: camera=(), microphone=(), display-capture=()` is set site-wide (Â§16.5) as defence in depth.
...
### 29.7 Recording / playback / snapshots / media-recorder â€” MVP posture
...
<!-- cctv-allow-forbidden-terms: END (Â§29 Not Supported) -->
