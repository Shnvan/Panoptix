...
Current implementation note: the local foundation supports signed in-memory WebSocket dispatch, ACK/reject receipt, heartbeat fallback delivery, and bounded edge reconnect/backoff behavior through test-scaffolded hooks. Persistent queues, audit integration for command dispatch/ACK, production reconnect supervision, and real camera/media actions are not implemented yet.
