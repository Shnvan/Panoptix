...
Current implementation note: the local foundation supports signed in-memory WebSocket dispatch, ACK/reject receipt, and heartbeat fallback delivery through test-scaffolded hooks. Persistent queues, audit integration for command dispatch/ACK, retry policy, full reconnect/backoff handling, and real camera/media actions are not implemented yet.
